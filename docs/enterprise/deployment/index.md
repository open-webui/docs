...
- **`/api/models`**: Lists the models the caller can use, which exercises the configured model backends. Requires an authenticated user token; API keys work only after an admin enables them (`ENABLE_API_KEYS`, off by default).
...
This auto-instruments FastAPI, SQLAlchemy, Redis, and HTTP clients, giving visibility into request latency, database query performance, and cross-service traces. `ENABLE_OTEL` alone exports nothing; each signal has its own switch (`ENABLE_OTEL_LOGS` for logs). The exporter speaks OTLP over gRPC by default, which is why the endpoint uses port 4317; set `OTEL_OTLP_SPAN_EXPORTER=http` to send to an HTTP collector on 4318 instead.
