...
- `WEBUI_ENABLE_SSO` → OAuth is on by default through `ENABLE_OAUTH`. `ENABLE_OAUTH_SIGNUP` is only needed to let SSO create new accounts
...
# Needed only for SSO to create accounts that do not exist yet
...
- **All OAuth providers:** `ENABLE_OAUTH` (on by default), plus `ENABLE_OAUTH_SIGNUP=true` when SSO should create accounts. `WEBUI_URL` only matters behind a proxy that changes the request URL
...
While it is `false` the OAuth section of the Admin Panel is read-only, so an edit cannot look like it worked and then vanish on the next restart. See [`ENABLE_OAUTH_PERSISTENT_CONFIG`](../reference/env-configuration.mdx#enable_oauth_persistent_config).
...
# Increase the OAuth handshake timeout (seconds). AIOHTTP_CLIENT_TIMEOUT does not
# affect the OAuth client, which is httpx-based.
OAUTH_TIMEOUT=60
```
...
# Needed only for SSO to create accounts that do not exist yet. OPENID_PROVIDER_URL is
# not required for Microsoft: login and sign-out use the tenant's own discovery URL.
ENABLE_OAUTH_SIGNUP=true
```
...
- `The email or password provided is incorrect` → Often means OAuth completed but session wasn't established. Token and claim failures are reported to the browser with this message, so read the backend log for the actual cause. A role mismatch or a signup-disabled case shows the access-prohibited text, a duplicate email shows the already-registered error, and other exceptions show `Error during OAuth process`
