...
#### CyberArk Identity, Apereo CAS and other providers with vendor-specific ID token headers:
...
```
OAuth callback error during authorize_access_token for provider oidc: OAuth callback failed: Unsupported {'app_id'} in header
```

The parameter name varies by provider (`app_id` for CyberArk Identity, `client_id` for Apereo CAS). It is a private parameter the provider adds to the JOSE header of the ID token, and older versions of Open WebUI rejected any header parameter that is not a registered one.

**Solution:** Update Open WebUI. Unrecognized header parameters are now ignored instead of ending the login, so no provider-specific configuration is needed and nothing has to be relaxed on the provider side. The signature check, the algorithm allowlist, `crit` handling and validation of the registered header parameters all still apply.
...
- `Unsupported {'app_id'} in header` → The provider adds a vendor-specific parameter to the ID token header and your version still rejects it. See [CyberArk Identity, Apereo CAS and other providers with vendor-specific ID token headers](#cyberark-identity-apereo-cas-and-other-providers-with-vendor-specific-id-token-headers)
