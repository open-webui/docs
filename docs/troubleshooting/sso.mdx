...
2. **For Production:** Either:
   - Set `ENABLE_OAUTH_PERSISTENT_CONFIG=true` and configure settings through the Admin Panel instead of environment variables, OR
...
:::info The Admin Panel tells you which one is in charge

While `ENABLE_OAUTH_PERSISTENT_CONFIG` is `false`, the **OAuth / OIDC** section of **Settings > Admin > Authentication** is read-only. The values are all visible, the controls do not accept input and a note above the section names `ENABLE_OAUTH_PERSISTENT_CONFIG`. Saving the page leaves the OAuth settings alone, so an edit can no longer look like it worked and then disappear on the next restart.

If you need to configure OAuth from the Admin Panel, set `ENABLE_OAUTH_PERSISTENT_CONFIG=true` and restart.
...
The parameter name varies by provider, `app_id` and `client_id` being two seen in the wild. Some providers add a vendor-specific parameter of their own to the header of the ID token they issue, and older versions of Open WebUI rejected any parameter they did not recognize there.
...
v0.6.41 moved each account's identity provider subject out of the old `oauth_sub` column into an `oauth` JSON field that can hold several providers per account. In builds from v0.9.6 onwards, the step that copied those values encoded them twice, so the field ended up holding the *text* of a JSON object rather than the object. Every read of such an account then failed validation, and the lookup that matches an incoming SSO login to an existing account never matched it.
...
- `The email or password provided is incorrect` → Often means OAuth completed but session wasn't established. Every callback failure is reported to the browser with this message, so read the backend log for the actual cause
- `Unsupported {'app_id'} in header` → The provider adds a vendor-specific parameter to the ID token and your version still rejects it. See [Providers that add their own parameters to the ID token](#providers-that-add-their-own-parameters-to-the-id-token)
- `1 validation error for UserModel` on the `oauth` field → An old upgrade step stored the identity provider subject as text. See [Existing Accounts Cannot Sign In After a Long-Delayed Upgrade](#12-existing-accounts-cannot-sign-in-after-a-long-delayed-upgrade)
...
| 🔄 Persistent config conflicts | Set `ENABLE_OAUTH_PERSISTENT_CONFIG=false` for env vars, or `true` to use the admin panel (it is read-only while `false`) |
...
| 🧬 SSO accounts locked out right after a long-delayed upgrade | Update Open WebUI; a repair migration fixes the stored identity data on startup |
