...
8.  **Presence Writes:** [`DATABASE_USER_ACTIVE_STATUS_UPDATE_INTERVAL`](/reference/env-configuration#database_user_active_status_update_interval) already throttles each user's `last_active_at` write to once per 60 seconds, so the flood of tiny `UPDATE`/`COMMIT` transactions is avoided by default. Set `120` if you want fewer writes still, which keeps a full minute of headroom inside the 180 second active-presence window. Anything at or above 180 drops users out of the count between writes, and `0` disables throttling and writes on essentially every request.
...
**Cause:**
- **Configuration Sync:** Replicas are not synced. Since v0.10.0 every replica reads configuration from the shared database on each access, so settings changes need no Redis. Redis provides cross-replica stop commands, the optional base-model cache (`ENABLE_BASE_MODELS_CACHE`), the session store when `ENABLE_STAR_SESSIONS_MIDDLEWARE` is on, and the default for `WEBSOCKET_REDIS_URL`. The in-memory model registry is shared only with `WEBSOCKET_MANAGER=redis`.
- **Missing Redis:** Configuration is read from the shared database on each access, so settings changes reach every replica without Redis. What stays local without `REDIS_URL` is the in-memory model registry (shared only with `WEBSOCKET_MANAGER=redis`), cross-replica stop commands, and the optional base-model cache.

**Solution:**
Set `REDIS_URL` to point to your shared Redis instance for those pieces. There is no configuration Pub/Sub to enable: the database already carries the settings.
...
1. **Migrate to PostgreSQL (strongly recommended, required for multi-replica):**
   ```bash
   DATABASE_URL=postgresql://user:password@postgres-host:5432/openwebui  # the async driver is psycopg v3 since v0.9.2. asyncpg is not installed, and a +asyncpg URL fails to load
   ```
...
See [DATABASE_POOL_SIZE](/reference/env-configuration#database_pool_size) for details. For comprehensive database optimization including caching, session sharing, and content extraction tuning, see the [Performance & RAM](./performance.md) guide.
...
Stop all replicas and workers, let one instance migrate, then bring the fleet back up on the new version. That costs a short full outage, and there is no general zero-downtime path across a schema change.
