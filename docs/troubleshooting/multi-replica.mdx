...
- **Encoded output carries raw UTF-8 rather than `\uXXXX` escapes.** Both are valid JSON and every client parses them identically, but the exact bytes differ, which matters only if something downstream checksums or string-compares encoded JSON. Three characters are the exception and stay escaped exactly as the standard library escapes them: line separator (`U+2028`), paragraph separator (`U+2029`) and next line (`U+0085`). Anything that reads a streamed response line by line, Open WebUI itself and any client reading the OpenAI-compatible stream, treats all three as a line break, so sending them raw would split a reply mid-stream and silently lose the piece it was in. They are common in text lifted from PDFs and word processor documents, which makes a model quoting an uploaded file back the realistic case. This applies to what is stored as well as what is sent: non-ASCII text saved in the database, and in Valkey chunk details, is held raw and takes a little less space. Anything written before you turned the setting on stays readable, turning it back off is equally safe and neither direction needs a migration.
