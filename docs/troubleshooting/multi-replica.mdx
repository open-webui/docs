...
A message sent while the shared model list is being rebuilt no longer fails either. A chat running on one of the user's own [Direct Connections](/features/chat-conversations/direct-connections) could previously error out, or spend the turn working from a mix of old and new entries, if a model was dropped from the list at the moment the request read it. Each request now reads one consistent copy of the list, and the cost of reading it no longer grows with the number of models, so on an instance carrying hundreds of models the background work a chat turn fires off (chat title, tags, follow-up suggestions, query rewriting, image prompts and context compaction) no longer holds up other people's responses.

If Redis is already configured and the model list still empties itself, the cause is probably not configuration sync at all but name resolution: on v0.11.0 the container could intermittently stop resolving the hostname of the Ollama or provider server, which reads as "Model not found". See [Intermittent Name Lookup Failures](/troubleshooting/connection-error#-intermittent-name-lookup-failures-often-reported-as-model-not-found).
...
- Images in a chat show an **Image unavailable** placeholder instead of the picture, intermittently, depending on which replica served the request.
...
| **Switch vector DB** | Set [`VECTOR_DB`](/reference/env-configuration#vector_db) to `pgvector`, `mariadb-vector`, `milvus`, `qdrant`, etc. | These are client-server databases, inherently multi-process safe |
...
:::danger Critical: Never Run Two Versions Against One Database
...
Version skew is a separate and harder failure. Migrations move the schema forward for the whole database at once, and the still-running old instances keep issuing queries written against the old shape. Whether that survives depends entirely on what the release changed. The upgrade to v0.11.0 only added columns and indexes, so an old instance left running through it kept working. A release that renames or drops something does not: v0.10.0 renamed the `config` table to `config_old` and replaced it with a per-key table, and any older instance still querying `config` fails from the moment the migration lands. A third kind leaves the schema alone and rewrites rows that are already stored, so the old instances go on reading the same columns and find different values in them. Releases are not labelled by which kind they are, and comparing column lists before and after the upgrade does not tell you either.
...
Back up the database before you start. Putting the old image back does not undo a migration: the schema stays where the migration left it, and rows a migration rewrote stay rewritten. Restoring a backup taken before the upgrade is the way back. See [Backup & Restore](/getting-started/updating#backup--restore).
...
Every Socket.IO event in a multi-replica deployment is published through Redis, and each one is encoded on the way out and decoded on the way in. In clustered deployments that encoding was the single largest cost measured on the workers handling live updates, which is why this setting exists. It switches the whole application from Python's standard-library `json` to [orjson](https://pypi.org/project/orjson/), a Rust implementation several times faster. orjson already ships as a pinned dependency, so there is nothing to install. Available from v0.11.0.

**What it covers:**

| Path | Why it matters at scale |
|---|---|
| Socket.IO payloads, including the ones published over Redis, plus Redis-backed session and collaborative-document state | The reason to turn this on. Every live update crossing replicas is encoded and decoded here |
| Saving and opening chats, and the rest of what is stored as JSON (user settings, configuration, folders, notes) | A whole conversation is encoded on every save and decoded again on every open, so the cost follows the length of the chat rather than the number of requests |
| Upstream provider responses (OpenAI-compatible and Ollama), including per-chunk parsing of streamed completions | Runs once per arriving token batch, so it scales with streaming volume rather than request count |
| The requests sent out to model providers (OpenAI-compatible, Ollama and Anthropic) | Each one carries the whole conversation or a whole embedding batch, so it is the largest single encode on a chat request |
| Incoming HTTP request bodies and outgoing `JSONResponse` bodies | The broadest surface, but individually the cheapest |
| The stored chunk details on the Valkey and Oracle 23ai vector stores | Read back once per result, so the cost follows how many chunks each knowledge base search returns |
| Results returned by built-in tools, and working out a user's permissions when they sign in, sign up or reload the page | Small payloads, but on paths every user hits over and over |
| The output a Jupyter server returns for code run in chat | Only when the code execution or code interpreter engine is set to Jupyter. The cost follows how much a run prints and how many images it produces, so the difference shows on chart-heavy or output-heavy runs |
...
orjson is also narrower in what it can be asked to do: it always writes compact output, and it has fixed rules of its own for values the standard library will not encode at all. Encoding that asks for something outside that, indented output or a custom rule for converting values before they are written, is handed to the standard library as well, so the request is honoured instead of being dropped and the result matches what the same code produces with the setting off.
...
See [`ENABLE_ORJSON`](/reference/env-configuration#enable_orjson) for the variable itself and [Scaling → Switch the JSON Encoder to orjson](/getting-started/advanced-topics/scaling#switch-the-json-encoder-to-orjson) for where it fits in a scaled rollout.
