...
| Assistant replies are randomly blank, worse with tools enabled | [Connection Errors → Blank Replies on Tool Calls](./connection-error#-blank-replies-when-the-model-uses-a-tool) |
...
| Intermittent "Model not found", model list emptying itself, `ClientConnectorDNSError` | [Connection Errors → Intermittent Name Lookup Failures](./connection-error#-intermittent-name-lookup-failures-often-reported-as-model-not-found) |
...
| `Could not read content from <url>`, or a YouTube video that will not attach | [RAG → Link and YouTube attachments](./rag#14-attaching-a-link-or-a-youtube-video-fails) |
...
| Reply on a Responses connection goes blank or stops part way when generation ends | [Open Responses → Providers that deviate from the format](/getting-started/quick-start/connect-a-provider/starting-with-open-responses#providers-that-deviate-from-the-responses-format) |
| Model on a Responses connection answers as if its own previous reply never happened | [Open Responses → Providers that deviate from the format](/getting-started/quick-start/connect-a-provider/starting-with-open-responses#providers-that-deviate-from-the-responses-format) |
...
| SSO login ends with "The email or password provided is incorrect" and the log shows `Unsupported {'app_id'} in header` | [SSO → Provider adds its own ID token parameters](./sso#providers-that-add-their-own-parameters-to-the-id-token) |
| Nobody who ever used SSO can sign in after an upgrade, log shows `1 validation error for UserModel` on `oauth` | [SSO → Accounts locked out after a long-delayed upgrade](./sso#12-existing-accounts-cannot-sign-in-after-a-long-delayed-upgrade) |
...
| An image in a chat shows "Image unavailable" | [Image Generation → General Issues](./image-generation#general-issues) |
...
| Guide | Covers |
| :--- | :--- |
| [Connection Errors](./connection-error) | HTTPS, CORS, WebSocket, reverse proxy, Ollama, DNS, SSL, Podman, MCP |
