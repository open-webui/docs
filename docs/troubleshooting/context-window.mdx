...
- **The checkpoint is not where the chat resumes for you.** The summary is written onto a message that is already in the conversation, and writing to a message that already exists leaves the chat's place alone, so compaction does not move you back to the middle of the conversation. Opening a compacted chat takes you to its most recent message, and a chat that was left sitting on a checkpoint is moved forward to its most recent message the first time it is opened.
- **A configurable model writes the summary.** By default it uses the same model selection as title and tag generation: your configured Task Model (`TASK_MODEL` / `TASK_MODEL_EXTERNAL`) if set, otherwise the chat's current model. You can give summarization a model of its own with [`CONTEXT_COMPACTION_MODEL`](/reference/env-configuration#context_compaction_model), or **Context Compaction Model** in the same admin panel. That is worth setting when your task model is a small one picked for titles, since the summary has to carry a large part of the conversation. Whatever is chosen, an unavailable model falls back to the task model selection and then to the chat model, so compaction still runs. Summary length is capped by that model's own `max_tokens` (default 1000), unless you configure task model parameters, in which case those replace the cap entirely (see the next point).
- **The summarizer's generation parameters are configurable.** [`TASK_MODEL_PARAMS`](/reference/env-configuration#task_model_params), or **Task Model Parameters** under **Tasks** in the same admin panel, sets the parameters sent with every background request, compaction summaries included. Setting anything there removes the built-in 1000-token limit, so put `max_tokens` in yourself if you still want one. Reach for it when a reasoning model is writing the summaries and the checkpoints read like half-finished thoughts: thinking counts against the same 1000 tokens, so the budget can be gone before any summary is written. See [Task Models](/features/administration/task-models#task-model-parameters).
