...
- **The checkpoint is not where the chat resumes for you.** The summary is written onto a message that is already in the conversation, and an update to an existing message does not move your place in the chat. Opening a compacted chat takes you to its most recent message, and a chat that was left sitting on a checkpoint is moved forward the first time it is opened.
