...
These come from the **model provider** (OpenAI, Anthropic, Google, your Ollama server, GLM-5.x, etc.), **not from Open WebUI**. The provider counted the tokens of everything you sent and rejected the request because it exceeds the model's context window.
...
1. **Every model uses a different tokenizer.** The token count for the same text differs between OpenAI (tiktoken), Anthropic, Gemini, GLM, Qwen, Mistral, and so on. A truly correct trimmer would need a per-model tokenizer for every provider in existence. Getting that wrong ships silent data corruption.
...
3. **Everyone wants a different truncation policy.** We have seen users ask for all of the following, and all of them are reasonable:
   ...
   - Per-model policies (keep 1M tokens for Gemini, 1M for GPT-5.6, 32k for smaller local models).
...
It is **disabled by default**. An administrator enables and tunes it in **Settings > Admin > Interface**:

- **Context Compaction**: the on/off toggle ([`ENABLE_CONTEXT_COMPACTION`](/reference/env-configuration#enable_context_compaction)).
- **Context Compaction Model**: the model that writes the summaries ([`CONTEXT_COMPACTION_MODEL`](/reference/env-configuration#context_compaction_model)). Leave it on **Current Model** to use the chat's own model.
...
- **Task Model Parameters**: the generation parameters the summarization request is sent with ([`TASK_MODEL_PARAMS`](/reference/env-configuration#task_model_params)), higher up the same page under **Tasks**. This is how you give the summary a larger `max_tokens` than the built-in 1000. The setting is shared with titles, tags and the other background tasks, so see [Task Models](/features/administration/task-models) before changing it.
...
- **It runs server-side, per request, and is invisible in the chat.** Your full conversation is still stored and displayed in the UI; compaction only changes what is sent to the model on a given turn. The chat shows a Compacting context toast that resolves to Context compacted or Context compaction failed, but the stored messages carry no marker.
...
- **The checkpoint is not where the chat resumes for you.** Opening a compacted chat takes you to its most recent message, and a chat left sitting on a checkpoint is moved forward the first time it is opened.
- **A configurable model writes the summary.** It uses the chat's own model unless you give summarization a model of its own with [`CONTEXT_COMPACTION_MODEL`](/reference/env-configuration#context_compaction_model), worth doing when your task model is a small one picked for titles. An unavailable compaction model falls back to the chat model. The task model is never consulted, so compaction still runs. Summaries stop at 1000 output tokens unless [`TASK_MODEL_PARAMS`](/reference/env-configuration#task_model_params) is set, which removes that limit entirely; put `max_tokens` in yourself if you still want one. Reach for it when a reasoning model is writing the summaries and the checkpoints read like half-finished thoughts, since thinking counts against the same budget. See [Task Models](/features/administration/task-models).
- **Your system prompt, tools, and skills are preserved.** Compaction only summarizes conversation messages. The model, workspace and folder system prompts, the available tools and their instructions, skills and any RAG, web search or memory context are all reassembled fresh on every request, after compaction runs, so they are never summarized away. The one exception is older **tool-call results** that live in the chat history: a past tool result inside the compacted block is condensed into the summary, though what the model can still call is unaffected. A system message already sitting at the head of the request is held aside before compaction runs and put back at the front afterwards, so it keeps its position rather than being counted among the messages that may be summarized or reordered behind the summary.
...
- **You can read the current usage over the API.** `GET /api/v1/chats/{id}` and `POST /api/v1/chats/{id}/compact` include a `context_usage` object, so a client can show how close a chat is to compacting:
  ...
  It is `null` when compaction is disabled or the chat has no messages. `source` is always `estimated`, even when the count was built from provider-reported usage, so it does not tell the two apart. Open WebUI prefers the token counts the provider itself reported: it walks back to the most recent message carrying a usage record, takes its prompt and completion totals, and only estimates the messages after that point. When no message carries usage, the whole figure is estimated, and an estimate is not a tokenizer-exact figure, so treat it as a gauge rather than a billing number.

  Provider vocabularies differ, and all the common ones are read: `prompt_tokens` / `completion_tokens` (OpenAI-style), `input_tokens` / `output_tokens`, `prompt_eval_count` / `eval_count` (Ollama), and `prompt_n` / `predicted_n` plus `cache_n` (llama.cpp). Local backends therefore compact on reported numbers rather than falling back to estimation, which previously made them drift from the threshold. The stored usage travels with the chat history, so the reported count is available on later turns too, not only on the request that produced it.
...
Enable this filter globally or attach it to specific models in **Admin Panel > Functions**. The `max_turns` valve is configurable per-model via the model card, so you can set a smaller window for local 8k models and a larger one for Gemini's 1M.
...
As of v0.9.6 Open WebUI reconciles these when it reconstructs a conversation: unpaired `tool_use` / `tool_result` entries are dropped before the request is sent, so resuming a chat with an interrupted tool call no longer hard-fails. Each call is kept only with its own result from the same reply. The saved chat itself stays as it is. Well-formed history is untouched. This is independent of the filter above: the filter still matters because *trimming* can create fresh orphans after reconstruction, which the server-side pass (run earlier, on the stored output) does not see. If you still hit this error, confirm you are on v0.9.6 or later.
:::
...
class Filter:
    ...
    def _budget_for(self, model_id: str) -> int:
        ...
        # Prefix match: "claude-sonnet-5-20260514" uses the "claude-sonnet-5"
        # budget. Longest key first so "gpt-5.6-sol-pro" beats "gpt-5.6-sol".
...
- **Prefix match on model id, longest-first.** `claude-sonnet-5-20260514` transparently uses the `claude-sonnet-5` budget. If you list nested ids like `gpt-5.6-sol` and `gpt-5.6-sol-pro`, `_budget_for` sorts keys by length descending before the prefix loop so the more specific one wins; otherwise dict insertion order would decide and `gpt-5.6-sol` could shadow `gpt-5.6-sol-pro` for anyone who listed it first.
...
Open WebUI doesn't always present the raw provider id to `body["model"]`. If an admin sets a connection `prefix_id`, every model is wrapped as `{prefix}.{raw_id}` (e.g. `openai.gpt-5.6-sol`). Pipe-function manifolds wrap their sub-models as `{pipe.id}.{sub_id}` (e.g. `anthropic.claude-sonnet-5-20260514`). Custom Workspace models can have arbitrary ids, often UUIDs.
