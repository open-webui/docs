...
- **Text-to-Speech Engine**: Choose between OpenAI-compatible, Mistral, ElevenLabs, Azure, local Transformers, or disable backend TTS (browser-only)
...
- **TTS Engine**: Choose **Kokoro.js (Browser)** for local in-browser TTS
...
**In Settings → Admin → Audio:**
- **STT Engine:** `OpenAI` | **Model:** `gpt-transcribe` (or the legacy `whisper-1`)
...
### Fast Setup: Mistral STT + TTS (Paid)

If you prefer Mistral's audio stack:

**In Settings → Admin → Audio:**
- **STT Engine:** `MistralAI` | **Model:** leave empty for `voxtral-mini-latest`, or name another Voxtral model (any id is passed through unchanged)
- **TTS Engine:** `MistralAI` | **Model:** `voxtral-mini-tts-2603` | **Voice:** choose from list
...
environment:
  ...
  - AUDIO_TTS_MODEL=voxtral-mini-tts-2603
```

→ See guides: [Mistral Voxtral STT](/features/chat-conversations/audio/speech-to-text/mistral-voxtral-integration) | [Mistral TTS](/features/chat-conversations/audio/text-to-speech/mistral-tts-integration)
...
- **TTS Engine:** `Web API` or `Kokoro.js (Browser)` (uses browser's built-in text-to-speech or client-side Kokoro; **does not call the backend TTS endpoint**)
...
```yaml
environment:
  - # Open WebUI has no EXTRA_PIP_PACKAGES hook. Build a derived image instead:
...
**Checklist:**
1. Verify the TTS engine is correctly configured in **Settings → Admin → Audio**
...
The most reliable fix is to upgrade to the latest version of Open WebUI. `WHISPER_COMPUTE_TYPE` defaults to `int8` on every image, the `:cuda` image included. There is no CUDA-specific default, so on a GPU set `WHISPER_COMPUTE_TYPE=float16` (or `int8_float16`) yourself.
...
- **CUDA mode:** The `:cuda` image defaults to `int8` like every other image, which can cause errors on older GPUs. Set `float16` explicitly for GPUs.
:::
...
- Text-to-speech has honored this setting since May 2025, and speech-to-text since v0.9.6 (before that STT ignored it and always verified). If it still appears ignored, confirm you are on v0.9.6 or later.
...
|----------|-------------|
| `AUDIO_TTS_ENGINE` | TTS engine: `""` (empty, disables backend TTS, uses browser), `openai`, `mistral`, `elevenlabs`, `azure`, `transformers` |
...
| `AUDIO_TTS_MISTRAL_API_KEY` | API key for Mistral TTS |
| `AUDIO_TTS_MISTRAL_API_BASE_URL` | Base URL for Mistral TTS |
