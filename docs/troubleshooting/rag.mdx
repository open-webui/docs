...
Typical for OpenAI (GPT-4o, GPT-5), Anthropic (Claude), Google (Gemini), DeepSeek, and other API providers where context windows are 32K to 1M+ tokens.
...
- **For Ollama Models**: Extend the model's context length:
  - Navigate to: **Settings > Admin > AI > Models**, then click the pencil (**Edit**) on the model you want to change
...
| 🗂 Single attached file returns nothing | Re-index to rebuild the per-file collections |
...
Because **Native Function Calling is now the default**, the model needs both the **ability** and the **instruction** to use the knowledge tools (unless you switch the model to Legacy).
...
1. **Check Global Model Settings first**:
   - Go to **Settings > Admin > AI > Models > Model Defaults > Configure**
...
You attached a webpage or a YouTube video to a chat (**+ > Attach Webpage**, or by typing the URL after `#` or `@`), and it came back as an error toast instead of a document.

**The Problem**: The message tells you what went wrong. A link that could not be read names that link, and a YouTube video whose transcript was refused gives the reason. A message about the knowledge base means the content was read and only saving it failed.

#### A link that could not be read

```
[ERROR: Could not read content from https://example.com/page]
```

Nothing was attached: the page could not be reached, or nothing readable came back. The URL in the message is the one that failed, which matters when you attach several at once. Common causes:

- The site blocks the request or serves its content only after JavaScript runs. Switch [`WEB_LOADER_ENGINE`](/reference/env-configuration#web_loader_engine) to `playwright`.
- Open WebUI sits behind a proxy it is not being told to use. See [`WEB_SEARCH_TRUST_ENV`](/reference/env-configuration#web_search_trust_env), which covers plain URL attachments as well as web search.
- The address is refused on purpose. Private and loopback addresses, cloud metadata endpoints and anything matched by [`WEB_FETCH_FILTER_LIST`](/reference/env-configuration#web_fetch_filter_list) are never requested.

#### A YouTube video whose transcript was refused

A YouTube attachment is the video's transcript, so a video with no usable transcript cannot be attached at all. The message says which of these happened:

| Message | What it means |
|---------|---------------|
| `YouTube blocked the transcript request for <video id> from this server. …` | YouTube refused the request coming from your server's address, which is common for data center and cloud provider ranges. Route these requests through a proxy: **Settings > Admin > Web Search > Youtube Proxy URL**, or [`YOUTUBE_LOADER_PROXY_URL`](/reference/env-configuration#youtube_loader_proxy_url). |
| `Transcripts are disabled for the YouTube video <video id>.` | The uploader turned captions off. No setting changes this. |
| `The YouTube video <video id> is age restricted, so its transcript cannot be retrieved.` | Age restricted videos do not return a transcript. |
| `The YouTube video <video id> is unavailable.` | The video is private, removed, region blocked or the ID in the link is wrong. |
| `YouTube requires additional verification to return the transcript for <video id>.` | YouTube asked for an extra verification step that Open WebUI cannot provide. |
| `No transcript found for the YouTube video <video id> in these languages: <codes>.` | The video has transcripts, just not in the requested ones. Add the language to [`YOUTUBE_LOADER_LANGUAGE`](/reference/env-configuration#youtube_loader_language). English is added to the end of the list when it is not already in it, so a video with only, say, Japanese captions needs `ja` added explicitly. |
| `Could not retrieve a transcript for the YouTube video <video id>.` | Any other transcript failure. Check the server log for the underlying error. |

:::note Same URL, less detail in a knowledge base
Adding a URL under **Workspace > Knowledge** reports `Failed to process URL: <url>` regardless of the cause. The specific reason is written to the browser console and the server log.
:::
...
Attaching one file to a chat with `#` retrieves nothing, while attaching the knowledge base that contains the same file works normally. The model then answers from general knowledge without flagging that it found nothing.

**The Problem**: A file that belongs to a knowledge base is indexed twice: once into its knowledge base's collection, and once into a per-file collection of its own (`file-<file id>`). Attaching the file on its own searches only the per-file collection, so if that collection went missing (through a vector database migration, a partial reset or a manual deletion) the search comes back empty while the knowledge base keeps working.

✅ **Solutions:**

1. **Re-index**: go to **Admin Settings > Documents > Reindex Knowledge Base Vectors** and click **Reindex**. This rebuilds each knowledge base collection and the per-file collection of every file inside it, which restores retrieval for the individual file. It re-embeds everything, so allow for the time and, with a paid embedding model, the cost.
2. **Re-upload the file** if it is not in any knowledge base. Re-indexing only walks knowledge bases, so a file that was only ever attached to a chat has to be uploaded again to get its collection back.
