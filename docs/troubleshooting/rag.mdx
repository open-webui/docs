...
**The Problem**: A file that belongs to a knowledge base is indexed twice, once as part of the knowledge base and once on its own. Attaching the file by itself searches only its own index, so if that has gone missing (after a vector database migration, a partial reset or a deletion made directly in the vector database) the search comes back empty while the knowledge base keeps working.

✅ **Solutions:**

1. **Re-index**: go to **Settings > Admin > Documents > Reindex Knowledge Base Vectors** and click **Reindex**. This rebuilds each knowledge base and the per-file index of every file inside it, which restores retrieval for the individual file. It re-embeds everything, so allow for the time and, with a paid embedding model, the cost.
2. **Re-upload the file** if it is not in any knowledge base. Re-indexing only covers knowledge bases, so a file that was only ever attached to a chat has to be uploaded again.
