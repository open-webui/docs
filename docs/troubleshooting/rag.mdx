...
The page was not fetched or not parsed, so nothing reached the vector database. The URL in the message is the one that failed, which matters when you attach several at once. Common causes:
...
- Open WebUI sits behind a proxy the loader is not using. See [`WEB_SEARCH_TRUST_ENV`](/reference/env-configuration#web_search_trust_env), which also governs the plain URL loader.
- The address is refused on purpose. Private and loopback addresses, cloud metadata endpoints and anything matched by [`WEB_FETCH_FILTER_LIST`](/reference/env-configuration#web_fetch_filter_list) are rejected before any request goes out.
...
| `YouTube requires additional verification to return the transcript for <video id>.` | YouTube demanded an extra proof-of-origin token that Open WebUI does not supply. |
