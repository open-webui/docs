...
Open WebUI resolves hostnames through the operating system resolver. v0.11.0 resolved them through c-ares instead, which the `aiodns` package activates inside aiohttp for the whole process with no way to opt out. c-ares does not behave the same everywhere:
