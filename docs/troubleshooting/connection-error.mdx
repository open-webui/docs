...
2. **A stale service worker serving a cached page.** Open WebUI registers a service worker, so the browser can display the cached UI shell while the live `/api/config` call fails, which produces this exact error. A normal hard refresh does not unregister the service worker. Clear it from **DevTools > Application > Service Workers > Unregister** for that origin, or open the instance in a fresh private/incognito window.
