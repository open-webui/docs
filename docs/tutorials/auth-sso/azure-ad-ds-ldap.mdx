...
```
Restart Open WebUI after making this change. For the Docker deployment this page targets, the host CA store is not visible inside the container: mount the certificate into the container and point `LDAP_CA_CERT_FILE` at that path instead, which is the setting the LDAP client actually reads (it is shown commented out in section 7).
