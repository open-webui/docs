...
5.  **Authorize:** (Method 1 Only) If this is your first time connecting via Streamable HTTP, you will be redirected to a "Connect with Notion MCP" screen.
    *   Ensure the correct **Workspace** (the one you configured in step 1) is selected in the dropdown.
    *   Click **Continue**.

:::note Frequent Re-authentication
...
This is **intended behavior** by Notion to keep your workspace secure. To fix it, revist steps 1-4 of this option to complete the "Connect with Notion MCP" authorization flow again, which will refresh your session.
:::
