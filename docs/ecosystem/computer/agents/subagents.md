...
Treat delegation as a way to gather parallel evidence, not a way to hide work. Give each worker a question, a boundary, and an expected result that can be checked in the parent workspace.
...
Each sub-agent produces an inspectable result or chat record. The main task can summarize those findings without losing the selected workspace. If results are incomplete or overlap, cancel the unnecessary work and continue with one bounded task rather than adding more workers. Background sub-agents are process-bound: Computer injects their result when they finish, but a restart interrupts active work rather than resuming it later.
