...
Want a ping? Add a notification target once, then just put the condition in the task's prompt ("notify me only if it fails"): the agent has a notify tool and decides when to use it. Blanket event pings on every finished/failed run exist too. See [notifications and webhooks](./notifications.md).
