...
You are making a school project, a portfolio piece, a club site, a zine, a small game, a craft plan, or another personal project with files you want to keep together. It is especially useful when you want to capture an idea on a phone and return to the same work on your computer later.
...
The workspace contains the expected folders, reference files, and project note. Open the same workspace from the computer after using another device: the note still says what to do next and the added materials are in the folder you chose. A saved voice memo, when used, appears as a real project file with its available transcript.

## If it did not

- If a shared item does not arrive, use the file browser upload action or create a note manually. Device sharing support varies.
- If a file appears in the wrong folder, verify it is the intended copy before moving it and correct the import preference before trying again.
- If voice recording or transcription is unavailable, save a typed note instead. Voice Memos must be enabled, and transcription needs a working configured service.

## Trust boundary

Everything you add becomes part of the real workspace on the host. A shared file may contain personal information, and voice transcription may send audio to its configured transcription provider. Keep the instance private, check where materials land, and avoid sharing a project folder with people you would not trust with its contents.
