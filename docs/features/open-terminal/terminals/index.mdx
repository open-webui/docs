...
Administration is a separate lane by design. The policy, lifecycle and refresh APIs are not terminal traffic, so they authenticate with the orchestrator's own `TERMINALS_API_KEY` and never with a user's token, whichever mode is in use. Keep that key set even in JWT mode, and use it when you call those endpoints yourself. Open WebUI attaches it to the policy and lifecycle calls it makes on your behalf when the connection's **Auth** is **Bearer**.
