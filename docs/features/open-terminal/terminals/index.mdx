...
[View on GitHub →](https://github.com/open-webui/terminals)
...
JWT mode needs the two sides to agree. The orchestrator checks each request against the identity of the user who made it, so Open WebUI has to send that user's own credentials rather than one shared key. That is what **Auth > Session** on the connection does, and it applies to the model's tools, the file browser and the terminal panel alike. Leaving the connection on **Auth > Bearer** in JWT mode fails authentication, which shows up as a terminal that closes the moment it opens.

Administration authenticates separately, by design. The policy, lifecycle, status and refresh APIs are not terminal traffic, so they take the orchestrator's own `TERMINALS_API_KEY` and never a user's token, whichever mode is in use. Keep that key set even in JWT mode, and send it when you call those endpoints yourself. Open WebUI attaches it to the policy and lifecycle calls it makes on your behalf when the connection's **Auth** is **Bearer**.
...
- If using **JWT mode**, confirm `TERMINALS_OPEN_WEBUI_URL` points to a reachable Open WebUI instance, and that the connection's **Auth** is set to **Session**.
...
Terminals requires an [Open WebUI Enterprise License](https://docs.openwebui.com/enterprise) for production use. See the [Terminals repository](https://github.com/open-webui/terminals) for details.
