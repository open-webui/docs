...
JWT mode needs the two sides to agree. The orchestrator checks the token against Open WebUI and then refuses the request unless the identity it gets back matches the user the request claims to be for, so Open WebUI has to send each user's own token rather than one shared key. That is what **Auth > Session** on the connection does, and it applies to the model's tools, the file browser and the interactive terminal alike. With **Auth > Bearer** the orchestrator sees the shared key instead of a user token and rejects it, which shows up as a `401` or as a terminal that closes the moment it opens.
