...
Open Terminal prints its address and key on startup:
...
Or skip the `cd` and point the server at the project with the `--cwd` flag:
...
With a project folder named `myproject`, Compose brings both services up together:
...
Open Terminal can also run as a [Model Context Protocol](https://modelcontextprotocol.io/) server with `open-terminal mcp`, exposing the terminal to MCP-compatible clients. This needs the optional `mcp` extra (which pulls in [FastMCP](https://github.com/jlowin/fastmcp)).
...
- [**Connect to Open WebUI →**](./connecting.md)
- [**Updating →**](./updating.mdx), how to move an existing install to a newer version.
