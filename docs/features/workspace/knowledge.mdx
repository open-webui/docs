...
- **A wide `grep` is refused rather than run.** Searching more than 200 files ([`KB_EXEC_MAX_GREP_FILES`](/reference/env-configuration#kb_exec_max_grep_files)) returns a prompt to scope the search to a directory or glob, and `grep` returns at most 50 matching lines ([`KNOWLEDGE_GREP_MAX_MATCHES`](/reference/env-configuration#knowledge_grep_max_matches)), the same cap `grep_knowledge_files` uses.
