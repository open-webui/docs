---
sidebar_position: 4
title: "Knowledge Bases and Document Chat"
sidebar_label: "Knowledge"
description: "Upload documents to a knowledge base, ask questions about their contents, and choose how to retrieve and share them in Open WebUI."
---

import ThemedImage from '@theme/ThemedImage';
import useBaseUrl from '@docusaurus/useBaseUrl';

# Knowledge Bases and Document Chat {#knowledge}
...
Use Knowledge Bases to ask questions across PDFs, documents, and other files in Open WebUI. Upload your files, attach the collection to a model or conversation, and ask a question. Where document content is processed depends on your extraction, embedding, and model providers; see [Chat Data Privacy & Encryption](/security/chat-data-privacy-and-encryption).

## Ask your first document question

Start with a running Open WebUI instance, a connected model, and permission to create knowledge bases and upload files. For focused retrieval, use a model that supports [Native function calling](/features/extensibility/plugin/tools#tool-calling-modes-default-vs-native), with **Builtin Tools** and the **Knowledge Base** tool category enabled. Native is the default mode; the model must use its tools to retrieve attached knowledge.
...
5. Compare the answer and any source references with the original document. If the model cannot find the content, follow [Knowledge Base troubleshooting](/troubleshooting/rag#13-knowledge-base-attached-to-model-not-working).

## Choose how to work with files

| Need | Use |
| :--- | :--- |
| Ask about one file in one conversation | [Upload a file to the chat](/features/chat-conversations/data-controls/files#using-files-in-open-webui) |
| Reuse a document collection across conversations | A Knowledge Base, using the steps above |
| Analyze spreadsheets or create files through code | [Open Terminal's file-analysis guide](/features/open-terminal/use-cases/file-analysis) |

Unlike [Notes](/features/notes), which inject full content into every message, Knowledge can retrieve relevant passages on demand. This makes it useful for document sets that would exceed the model's context window if sent in full. For local-only document chat, also follow [Run Open WebUI Offline](/tutorials/maintenance/offline-mode).
...
| **Result cap** | The tool's own `count` argument, which the model sets (default 5). The admin Top K setting does not apply here | 50 matches ([`KNOWLEDGE_GREP_MAX_MATCHES`](/reference/env-configuration#knowledge_grep_max_matches)) |
...
2. **Check the model's tool capabilities and retrieval setup** using [Knowledge Base troubleshooting](/troubleshooting/rag#13-knowledge-base-attached-to-model-not-working). Keep Native mode enabled and use a model that can call tools reliably.
...
For the initial create, upload, and chat steps, see [Ask your first document question](#ask-your-first-document-question). The sections below cover managing the collection after setup.
