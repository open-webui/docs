...
- It completes only for the account that began it. If a different account is signed in when the provider redirects back, the authorization is refused and no account is connected.
- Signing out cancels an authorization that is still in progress. Sign back in and enable the tool again to start over.
- Finish the flow in the same browser and at the same address you started it from. Coming back anywhere else leaves nothing for the authorization to match against.
...
| `OAuth callback failed: OAuth callback user does not match authenticated session` | A different user is signed in than the one who started the authorization. |
