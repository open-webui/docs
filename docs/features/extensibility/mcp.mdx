...
:::tip MCP tools need a capable model
MCP connects Open WebUI to your tools; it does not improve the model's ability to use them. Even with a working connection, a model may ignore tools, send incorrect arguments, or fail to complete a multi-step task. **Try a stronger model with reliable native tool calling** if this happens. Poor tool use alone does not mean Open WebUI or the MCP integration is broken. See [troubleshooting model tool use](/features/extensibility/plugin/tools/openapi-servers/open-webui#tools-are-enabled-but-the-model-does-not-use-them-correctly).
:::
