...
:::tip Tool reliability depends on your model
Connecting an external tool makes it available to the model; the model still has to choose the right tool, supply valid arguments, and use the result. A model that supports tool calling can still struggle with these steps, especially across multiple calls. If tools are enabled but the model ignores them or calls them incorrectly, **try a more capable model with reliable native tool calling** before assuming Open WebUI is at fault. See [troubleshooting model tool use](/features/extensibility/plugin/tools/openapi-servers/open-webui#tools-are-enabled-but-the-model-does-not-use-them-correctly).
:::

:::tip A first-party example
The [oikb](/ecosystem/knowledge-base-sync) sync daemon is a ready-made OpenAPI tool server. Run it, add its URL under **Settings → Integrations**, and a model can trigger Knowledge Base syncs and check their status from chat. See [Let the model trigger syncs](/ecosystem/knowledge-base-sync/daemon#let-the-model-trigger-syncs).
:::
...
- **One-way events only**: OpenAPI tools can emit status updates, notifications, and other [one-way events](/features/extensibility/plugin/development/events#-external-tool-events) via the REST endpoint. Open WebUI passes the `X-OpenWebUI-Chat-Id` and `X-OpenWebUI-Message-Id` headers to enable this, but only when [`ENABLE_FORWARD_USER_INFO_HEADERS`](/reference/env-configuration#enable_forward_user_info_headers) is turned on, which it is not by default. However, interactive events (user input prompts, confirmations) are only available for native Python tools.
