...
This documentation explains how to easily set up and deploy the [**MCP (Model Context Protocol)-to-OpenAPI proxy server** (mcpo)](https://github.com/open-webui/mcpo) provided by Open WebUI. Learn how you can expose MCP-based tool servers using standard, familiar OpenAPI endpoints suitable for end-users and developers.
...
Set `WEBUI_SECRET_KEY` in your Open WebUI Docker config and keep it stable. If you connect this proxy with OAuth, its stored client info and session tokens are encrypted with a key derived from `WEBUI_SECRET_KEY`, so a key that changes on container restart makes them undecryptable and the connection has to be re-authorised. A static API key is stored as-is and is unaffected.
:::

### 📌 What is the MCP Proxy Server?

The MCP-to-OpenAPI proxy server lets you use tool servers implemented with MCP (Model Context Protocol) directly via standard REST/OpenAPI APIs, with no need to manage unfamiliar or complicated custom protocols. If you're an end-user or application developer, this means you can interact easily with powerful MCP-based tooling directly through familiar REST-like endpoints.
...
The mcpo proxy addresses those issues:
...
- 🔌 Uses plain HTTP, with no socket setup, daemon juggling, or platform-specific glue code
...
✨ With mcpo, your local-only AI tools become cloud-ready, UI-friendly, and instantly interoperable, typically without changing your tool server code.

:::tip
**Events Supported**: While mcpo exposes MCP tools as OpenAPI endpoints, they can still emit events (status updates, notifications) via Open WebUI's [event REST endpoint](/features/extensibility/plugin/development/events#-external-tool-events). Open WebUI passes the required `X-OpenWebUI-Chat-Id` and `X-OpenWebUI-Message-Id` headers to your tool when [`ENABLE_FORWARD_USER_INFO_HEADERS`](/reference/env-configuration#enable_forward_user_info_headers) is turned on, which it is not by default. However, interactive events requiring user input are only available for native Python tools.
:::
...
🤝 **To integrate with Open WebUI after launching the server, check our [docs](/features/extensibility/plugin/tools/openapi-servers/open-webui).**
...
No complex setup: just REST APIs ready to use.
...
✔️ Your production MCP servers are now available via REST APIs!
...
- **Asynchronous & Performant**: Built on robust asynchronous libraries, designed for speed and reliability with concurrent users.
...
- **Powerful & Automatic Docs**: Built-in Swagger UI documentation is automatically generated, up-to-date, and maintained.
...
- **Established Security & Stability**: Inherits well-established HTTPS transport, standard auth methods (JWT, API keys), solid async libraries, and FastAPI's robust framework.
- **Future-Proof**: MCP proxy uses existing, stable, standard REST/OpenAPI formats with broad community support and ongoing evolution.

🌟 **Bottom line:** MCP-to-OpenAPI makes your powerful MCP-based AI tools broadly accessible through intuitive, reliable, and scalable REST endpoints. Open WebUI proudly supports and recommends this approach.
