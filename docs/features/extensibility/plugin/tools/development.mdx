...
Two rules follow from how the class is loaded. Every public callable on the instance is exposed to the model as a tool, so helper methods must start with `_`. And `__init__` has to set `self.valves = self.Valves()`: stored admin valves are only loaded onto an instance that already has a `valves` attribute, so without that line the tool runs on the class defaults no matter what the admin saved.

### Method Docstrings

The docstring of each method is what the model reads before deciding to call it. The text above the first `:param` becomes the tool's own description, and each `:param name:` becomes the description of that argument in the JSON schema.

A parameter description can run over as many lines as it needs. Every following line belongs to it until the next `:param`, or until another field such as `:return:`. Indentation is stripped and blank lines are dropped, so you can lay a long description out as a paragraph and the model still receives all of it:
...
Here the model is told about the operators and the quoting rule as well as the first line. The text after `:return:` is not sent to the model, so put anything the model needs to know into the description or into a `:param`.

Only the reST `:param name:` form is read. A Google-style `Args:` block produces no parameter descriptions and lands in the tool's own description instead.

### Type Hints
Each tool must have type hints for arguments. The types may also be nested, such as `queries_and_docs: list[tuple[str, int]]`. Those type hints are used to generate the JSON schema that is sent to the model. An argument without a type hint is typed as a string in the schema, so the model will always send a string. `Optional[X]` collapses to `X`, and a `None` default is dropped. Parameters whose names start with `__` are stripped from the schema along with their `:param` lines, so the model never sees the reserved arguments listed below.

### Return Values
A tool may return a `str`, a `dict` or a `list`. A `dict` is JSON-encoded for the model, a bare `list` is wrapped as `{"results": [...]}`, and anything else is converted with `str()`. A string that starts with `data:image/` is detached as an image attachment, and the model is told `<name>: Image file read successfully.` instead.

### Valves and UserValves (optional, but HIGHLY encouraged)

Valves and UserValves are used for specifying customizable settings of the Tool, you can read more on the dedicated [Valves & UserValves page](/features/extensibility/plugin/development/valves). Secrets stored in Valves (such as API keys) can be [encrypted at rest](/features/extensibility/plugin/development/valves#encrypting-valve-values-at-rest).

### Optional Arguments
Below is a list of optional arguments your tools can depend on:
- `__event_emitter__`: Emit events (see following section)
- `__event_call__`: Same as event emitter but can be used for user interactions. It waits for the browser to answer, with no timeout unless [`WEBSOCKET_EVENT_CALLER_TIMEOUT`](/reference/env-configuration#websocket_event_caller_timeout) is set to a number of seconds.
...
- `__model__`: A dictionary with model information. For workspace tools this is the task model when one is configured under **Admin Panel > Settings > Interface**, and the chat model otherwise. Read `__metadata__["model"]` for the model the user is chatting with.
- `__id__`: The tool's own id
- `__chat_id__` and `__message_id__`: The current chat and message ids
- `__request__`: The FastAPI request
...
Just add them as argument to any method of your Tool class just like `__user__` in the example above. `__messages__` and `__files__` are refreshed before every native tool call. The others are fixed when the request starts.

#### `file_handler`

A module-level `file_handler = True` tells Open WebUI to drop the attached files from the request once the tool has run, so the retrieval pipeline does not inject them as well. Only the Legacy function-calling path reads the flag. The Native loop ignores it.
...
1. **Administrator Level**: Go to **Settings > Admin > Models**, click the pencil (**Edit**) on the model, then **Advanced Params > Function Calling** (set to "Native" or "Legacy").
...
:::info `self.citation` no longer does anything
Older guides tell you to set `self.citation = False` to stop automatic citations overriding your custom ones. The flag is still read off the module, but nothing acts on it any more. In Legacy mode every tool result becomes a source regardless of the flag; in Native mode only a fixed set of builtin tools produces sources, and a Python tool's result never does. Setting it is harmless and leaving it out changes nothing.
:::
...
await __event_emitter__({
    ...
    "data": {"files": [{"type": "file", "name": "report.pdf", "url": "/files/report.pdf"}]}
})
```
...
The next two, `confirmation` and `input`, wait for the browser to answer, so they need `__event_call__`; through `__event_emitter__` the reply has nowhere to go. `execute` works with either: `__event_call__` when you want the result back, `__event_emitter__` for fire and forget.

**Confirmation Events**
```python
confirmed = await __event_call__({
    "type": "confirmation",
    "data": {"title": "Please confirm", "message": "Are you sure you want to continue?"}
})
```
...
answer = await __event_call__({
    "type": "input",
    "data": {"title": "More detail needed", "message": "Please enter additional information:", "placeholder": "..."}
})
```
...
result = await __event_call__({
    "type": "execute",
    "data": {"code": "return document.title;"}  # runs in the browser, so JavaScript
})
```
...
pip runs as a child process on a worker thread, so the rest of Open WebUI keeps serving requests during the install. Only the save that triggered it waits.

Open WebUI also rewrites four import prefixes in the saved source, `from utils`, `from apps`, `from main` and `from config`, to their `open_webui.` forms, as plain substring replacements. `from configparser import ConfigParser` therefore becomes `from open_webui.configparser import ConfigParser` and fails. Write `import configparser` instead, and avoid those four prefixes anywhere in the file.
