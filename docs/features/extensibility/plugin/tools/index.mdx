...
The UI is the recommended route, but if you configure your instance entirely through env vars you can set the same global default with `DEFAULT_MODEL_PARAMS`:

```yaml
DEFAULT_MODEL_PARAMS: '{"function_calling": "native"}'
```

This is equivalent to setting **Function Calling = Native** in the global model parameters panel above: it becomes the default for every model that hasn't overridden it. There is **no** standalone `FUNCTION_CALLING_MODE` (or similar) variable — `function_calling` is a key inside `DEFAULT_MODEL_PARAMS`.
:::
