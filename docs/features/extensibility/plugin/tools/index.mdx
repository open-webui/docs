...
| **Skills** | *Requires at least one skill to be attached to the model via **Workspace or Settings > Admin > AI > Models → Edit → Skills**. The model receives a summary of attached skills in its system prompt and can call `view_skill` to load full instructions on demand. No separate builtin tools category checkbox is needed; attaching a skill is the only requirement.* |
...
| `update_automation` | `automation_id` (required), `name`, `prompt`, `rrule`, `model_id`, `folder_id` (all optional; only provided fields are changed) | `{status, id, name, model_id, folder_id, is_active, next_runs}` |
