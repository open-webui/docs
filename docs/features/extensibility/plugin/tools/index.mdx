...
| `update_tasks` | Create/update a structured task list for the current chat, including status tracking (`pending`, `in_progress`, `completed`, `cancelled`). |
| **Automations** | *Requires per-model "Automations" category enabled (default: on) AND user has `features.automations` permission (admins always pass).* |
| `create_automation` | Create a scheduled automation with a name, prompt, RRULE schedule, and optional model ID. |
...
| **Task Management** | | |
| `update_tasks` | `tasks` (required), `overwrite` (default: true) | `{tasks: [...], summary: {total, pending, in_progress, completed, cancelled}}` |
| **Automations** | | |
| `create_automation` | `name` (required), `prompt` (required), `rrule` (required), `model_id` (optional — defaults to current chat model) | `{status, id, name, model_id, is_active, next_runs}` |
...
| **Task Management** | `update_tasks` | Create and maintain a structured task/todo list in the active chat |
...
For long-running workflows, combine interleaved tool use with `update_tasks` so the model can plan explicitly, track progress, and keep users aligned on next actions.
