...
At the storage layer, SQLAlchemy is now used in its async mode: sessions are `AsyncSession` instances, and queries are issued via `await db.execute(select(...))` instead of the legacy `db.query(...).first()` style. The sync session helper has been replaced by an async context manager.
...
3. **Database sessions are now `AsyncSession`.** The sync session helper `get_db_context` has been removed in favor of `get_async_db_context`, and any direct query code needs to be rewritten to the async SQLAlchemy style.
...
### 🔁 5. Third-party libraries that don't yet support async
...
- Replace `get_db_context` with `get_async_db_context` and rewrite raw queries in the SQLAlchemy 2.0 async style (`select(...)` + `await db.execute(...)`).
- Session type is now `AsyncSession`, not `Session`.
- Plugin entrypoint signatures don't change — only the code inside them.
