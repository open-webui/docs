...
2. **Featured does not mean vetted.** A plugin being highlighted, featured, popular, or listed on the [Open WebUI Community site](https://openwebui.com/) is **not** an endorsement and does **not** mean its code was reviewed for security or quality. Listings are community-submitted and unaudited. See [Community Plugins](/features/extensibility/community).

3. **Review code before importing.** Before installing any community Tool or Function, review its source code. If you don't understand what it does, don't install it.

4. **Protect your data directory.** The `data` directory (mounted at `/app/backend/data` in Docker) contains your database, configurations, and cached Tools/Functions. If an attacker gains write access to this directory, they can inject malicious code that will execute on your server.

5. **Restrict Workspace access.** Only trusted administrators should have permission to create, import, or modify Tools and Functions. Regular users should **not** have Workspace access unless explicitly required.

6. **Audit installed plugins regularly.** Periodically review the Tools and Functions installed in your instance via **Workspace → Tools** and **Admin Panel → Functions**.
...
- **Functions** extend the capabilities of the Open WebUI itself, enabling you to add new AI model support (like Vertex AI) or improve usability (like creating custom buttons or filters).
...
1. Add a new AI model provider like **Vertex AI** to the WebUI.
