...
- The Python stdlib, plus everything pip-installed in the container. Only what you declare under `requirements:` is yours to rely on. Packages Open WebUI happens to install come and go between releases, so a plugin that imports one without declaring it can stop loading after an update.
