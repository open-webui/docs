...
- The Python stdlib, plus everything pip-installed in the container. Only what you declare under `requirements:` is yours to rely on. Packages Open WebUI happens to install come and go between releases: `langchain-community` is gone, so a plugin importing `langchain_community` without declaring it fails to load.
