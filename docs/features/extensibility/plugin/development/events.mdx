...
| Header | Description |
|--------|-------------|
| `X-OpenWebUI-Chat-Id` | The chat ID where the tool was invoked |
| `X-OpenWebUI-Message-Id` | The message ID associated with the tool call |
...
By default `__event_call__` waits **forever** for a user response, so a prompt nobody answers holds that call open indefinitely. Set [`WEBSOCKET_EVENT_CALLER_TIMEOUT`](/reference/env-configuration#websocket_event_caller_timeout) to a number of seconds to bound it. Both failure modes come back as ordinary return values, never as exceptions, so check the result for an `error` key:

| Situation | What `__event_call__` returns |
|-----------|-------------------------------|
| The browser already disconnected | `{"error": "Client session disconnected."}`, straight away |
| Nobody answered before the timeout elapsed | `{"error": "Event call timed out. The browser tab may be inactive or closed."}` |

A timeout does not disconnect the tab. If the user simply had not answered yet, their session stays live and you can prompt them again.

```python
result = await __event_call__(
    {
        "type": "confirmation",
        "data": {"title": "Delete the file?", "message": "This cannot be undone."},
    }
)

if isinstance(result, dict) and result.get("error"):
    return f"Could not ask the user: {result['error']}"
```
:::
...
| `chat:title`                                 | Set (or update) the chat conversation title          | The title string itself                                                                              |
| `chat:tags`                                  | Tell the browser to reload the chat's tags           | Ignored. Write the tags to the database first, then emit                                             |
...
Additionally, when the message has no content yet and the last status has `hidden: true` (or no status exists at all), the frontend shows a blinking typing cursor instead of the status bar, so hidden statuses don't replace the indicator that something is still running. A visible status suppresses that cursor while the message is still empty; once the first content arrives the cursor trails the text until the message is marked done, whether a status is on screen or not.
...
By default the new entries are merged with whatever is already stored on the message, so repeated emits stack up in the saved chat, with the newest batch first. The live view is not cumulative: the browser replaces what it is showing with each payload, and the stacked array only appears after a reload. For a single live widget you want to update in place, set `data.replace: True`; the entire `embeds` array on the message is overwritten with the payload's array. (Pass an empty list with `replace: True` to clear all embeds for the message.)

| Mode | Behavior | Typical use |
| :--- | :--- | :--- |
| `replace` omitted / `False` | Each new payload's items are merged into `message.embeds`, newest first. | One-shot tools / actions that emit a single embed at the end. |
...
- **Sandbox and sizing.** Each embed is rendered inside a sandboxed iframe with `allow-scripts` and `allow-downloads` enabled, plus `allow-popups` for embeds attached to an assistant message. Same-origin access is user-toggled; form submissions are always allowed on that surface, whatever the user setting says. For auto-resize behavior, height reporting, and the Chart.js / Alpine auto-injection details, see [Rich UI Embedding → Sandbox and Security](/features/extensibility/plugin/development/rich-ui#sandbox-and-security).
...
**Update the chat's title.** `data` is the title string itself, which is also how Open WebUI emits it after title generation. An object here would set the title to that object.

```python
await __event_emitter__(
    {
        "type": "chat:title",
        "data": "Market Analysis Bot Session",
    }
)
```
...
**Refresh the chat's tags.** The browser ignores `data` on this event and reloads the chat and the tag list from the server, which is also how Open WebUI uses it after tag generation. Write the tags to the database first, then emit the event as the refresh signal:

```python
from open_webui.models.chats import Chats

await Chats.update_chat_tags_by_id(chat_id, ["finance", "AI", "daily-report"], user)
await __event_emitter__({"type": "chat:tags", "data": {}})
```
...
await __event_emitter__(
    {
        ...
        "data": {
            "source": {"name": "My Source Name", "id": "my-source-id"},
            "document": ["The exact text passage being cited."],
            "metadata": [
                {
...
`document` and `metadata` are parallel lists (one entry per cited passage). `source.name` / `source.id` label the citation group, and each `metadata` entry's `source` / `url` is the link shown under the response. External tools (OpenAPI and MCP) emit this same `source` event by POSTing the `{"type": "source", "data": {...}}` body to the [event endpoint](#-external-tool-events); this is how an MCP search tool attaches citations, including in native function calling mode.
...
Notifications are dropped for internal requests, so an automation or a background task never shows a toast.
...
**Where it appears:**
*   **Chat Overview**: The message's node in the conversation overview shows a filled heart, making it easier for users to locate key information later. There is no favorite control in the message toolbar itself.
...
To offer a fixed set of choices instead of free text, pass `"input": {"type": "select", "options": ["a", "b"]}` in the data payload. Cancelling the dialog returns `false` rather than a string, so check for it before using the value.
...
:::warning
If the JavaScript throws, the browser logs the error and never sends a reply, so an `__event_call__` waits until `WEBSOCKET_EVENT_CALLER_TIMEOUT` expires, which by default is forever. Wrap the code in `try` and `catch` and `return` an error value from the `catch` branch.
...
- **Extensibility:** The event system is designed to evolve, so always check the [Open WebUI documentation](https://github.com/open-webui/open-webui) for the most current list and advanced usage.
...
# response is the raw string the user typed
```
...
The title, yes: emit `"chat:title"` with the new title string as `data`. Tags are different. `"chat:tags"` only makes the browser reload the tags from the server, so write them to the chat first and then emit it.
...
Without this, Open WebUI does not include the identification headers. The endpoint itself stays available, and for a global tool server there is a narrower alternative: a per-connection custom header using the `{{CHAT_ID}}` and `{{MESSAGE_ID}}` placeholders is interpolated regardless of this setting, and unlike the flag it does not also ship the user's email, name and role to every external backend. Custom headers are not sent for user-added tool servers, which the browser calls directly.
...
|--------|-------------|------------------|
| `X-OpenWebUI-Chat-Id` | The chat ID where the tool was invoked | `FORWARD_SESSION_INFO_HEADER_CHAT_ID` |
| `X-OpenWebUI-Message-Id` | The message ID associated with the tool call | `FORWARD_SESSION_INFO_HEADER_MESSAGE_ID` |
...
- **Admin / service-account key (recommended)**: provision a dedicated admin (or service-account) user in Open WebUI, generate an API key for it, and use that key from the tool server. An admin key works for any user's chat, so a single key serves all callers; the forwarded `X-OpenWebUI-Chat-Id` + `X-OpenWebUI-Message-Id` headers tell your tool *which* chat/message to post to.
...
There is **no timeout** on pipe, tool, or action execution. Your code can run for minutes or hours, and nothing in Open WebUI will kill it automatically. The only things that can stop a running task are:
- The function itself returning or raising an exception
- Manual cancellation. The stop button in the UI calls `POST /api/tasks/chat/{chat_id}/stop`; the per-task `POST /api/tasks/stop/{task_id}` is admin-only and is the fallback when there is no chat id
...
Nothing below is written for a temporary chat or a channel message, since neither has a saved chat to write to.
...
| `embeds` | Merged into the message's `embeds` array (Rich UI HTML) newest first, or replaced when `data.replace: true` is set. See [`embeds`](#embeds-or-chatmessageembeds) for the full payload shape and a live-progress example. |
| `files` | Merged into the message's `files` array, newest first |
| `source` / `citation` | Appended to the message's `sources` array, but only when the payload has no `type` field of its own. A typed source such as `code_execution` is not written by the event emitter; it reaches the browser, which stores it on the message when it saves the chat. |

These 6 types write to the database inside the event emitter function itself, completely independent of `ENABLE_REALTIME_CHAT_SAVE`.
...
#### ⚠️ Requires live connection (errors on tab close)

| Type | Why |
|------|-----|
| `confirmation` | Uses `sio.call()`, waits for a client response and returns an error dictionary if the timeout elapses |
| `input` | Uses `sio.call()`, waits for a client response and returns an error dictionary if the timeout elapses |
| `execute` via `__event_call__` | Uses `sio.call()`, waits for a client response and returns an error dictionary if the timeout elapses |
| `execute` via `__event_emitter__` | Fires and forgets, **will not error**, but JS may not run if no browser is connected |

`confirmation` and `input` fundamentally require a live browser connection via `__event_call__`. If the tab is closed the session is gone, so the call returns `{"error": "Client session disconnected."}` straight away. If the tab is merely inactive the call waits for [`WEBSOCKET_EVENT_CALLER_TIMEOUT`](/reference/env-configuration#websocket_event_caller_timeout). That is unset by default, so it waits indefinitely; set it to a number of seconds and once it elapses the call returns `{"error": "Event call timed out. The browser tab may be inactive or closed."}`. Neither case raises, so test the return value instead of relying on `try`/`except`, and note that a timeout leaves the browser session connected: a user who was slow to answer can still be prompted again.
...
:::caution
The return value takes precedence over event-emitted content. If your pipe emits `"message"` events but returns `None`, the saved content will be empty, since the frontend's final save overwrites whatever the event emitter wrote to the database. Always return or yield your content directly from the `pipe()` method.
:::
...
Actions are the exception to the rules above. Only an inline `HTMLResponse` is written server-side; everything else is handed back to the browser, which decides what to do with it:

| Return type | What happens | Persisted? |
|-------------|-------------|-----------|
| `HTMLResponse` (with `Content-Disposition: inline`) | HTML body extracted → added to `embeds` → emitted as `"embeds"` event | ✅ Yes |
| `HTMLResponse` (without inline) | Body decoded to a string, then handled as any other return below | ❌ No |
| `{"messages": [...]}` | The browser merges each entry into the message with that `id` and saves the chat | ⚠️ Only if the tab is still open |
| any other `str` / `dict` | Returned to the caller and dropped, the chat is not touched | ❌ No |

#### Filters

Filters transform `form_data` in the pipeline, so they don't return results to the user directly. However, filters **do receive `__event_emitter__`** and can emit persisted event types like `"status"`, `"embeds"`, `"message"`, etc.
...
1. **Always return your final answer** from your `pipe()` or tool function, where the return value is saved. Actions are the exception: only a `{"messages": [...]}` return reaches the chat, and only the open tab writes it, so use an inline `HTMLResponse` or an event if it has to survive a tab close
