...
When developing your own `Tools`, `Functions` (`Filters`, `Pipes` or `Actions`), `Pipelines` etc, you can use special arguments to explore the full spectrum of what Open WebUI has to offer.
...
:::note The `body` differs between a Filter's `inlet()` and `request()`
`inlet()` sees the body early, before the pipeline has run; `request()` sees it last, as it leaves for the provider. Keys such as `features`, `variables`, `tool_ids`, `files`, `folder_id` and `skill_ids` have been popped off by then, while retrieved RAG sources and the resolved `tools` array have been added. The example below is an `inlet()`-shaped body. See [Where `request` sits in the pipeline](/features/extensibility/plugin/functions/filter#where-request-sits-in-the-pipeline).
:::
...
A `dict` with user information. It is the whole user record as `UserModel.model_dump()` returns it: `id`, `email`, `username`, `role`, `name`, `profile_image_url`, `profile_banner_image_url`, `bio`, `gender`, `date_of_birth`, `timezone`, `presence_state`, `status_emoji`, `status_message`, `status_expires_at`, `info`, `settings`, `oauth` (one entry per provider holding the `sub` the provider assigned), `scim`, `last_active_at`, `updated_at`, `created_at` and `is_active`. The user's `variables` are excluded from the dump. The example below shows only the fields most plugins read.
...
```json
{
  ...
  # the caller's User-Agent header, or "" when absent.
  "internal": false,
  # true when the server made the request itself (a task or an automation), not a client.
  "user_message": "[the latest user message dict from body['messages']]",
  "user_message_id": "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
  "assistant_message_id": "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
  "automation_id": null,
  # set when an automation started the turn.
  "folder_id": null,
  "filter_ids": [],
  # filters selected for this request, on top of the model's own.
  "chat_variables": {},
  # variables stored on the chat itself, as opposed to the request-level "variables" below.
  ...
  "params": {
    ...
    # "full" unless tool permissions are enabled and the request chose otherwise.
    # Always "full" for automations and channel messages.
  },
  "tools": "[the same dict as __tools__, present once tools have been loaded]",
  "task": "[the task name for internal task calls, see __task__ below]",
  "task_body": "[the task's body for internal task calls, see __task_body__ below]"
}
...
Pipes, Tools and a Filter's `inlet()` and `request()` receive it, for invocations that originate from a chat context, including:
...
It returns whatever the browser sent back. When it cannot get an answer it returns a `dict` with an `error` key instead of raising: `{"error": "Client session disconnected."}` if the tab is already gone, or `{"error": "Event call timed out. The browser tab may be inactive or closed."}` if [`WEBSOCKET_EVENT_CALLER_TIMEOUT`](/reference/env-configuration#websocket_event_caller_timeout) is set and elapses. Without that variable there is no timeout at all and the call waits indefinitely.
...
The dict is assembled by `get_tools()` in [tools.py](https://github.com/open-webui/open-webui/blob/main/backend/open_webui/utils/tools.py).

### `__oauth_token__`

A `dict` with the signed-in user's OAuth token data (`access_token` and the rest of what the provider returned, refreshed when it has expired), or `None` when the user did not sign in through OAuth or no session token is stored. Pipes, Tools and a Filter's `inlet()` and `request()` receive it. The token comes from the browser's OAuth session cookie, and falls back to the user's most recent stored OAuth session for automations and API calls made without one.

### `__id__`

The `str` id of the plugin itself: the tool id for Tools, the filter id for Filters, and the action id for Actions (the sub-action id when a sub-action was clicked). Pipes do not receive it.

### `__body__`

Filters only. `stream()` receives each chunk as `event` rather than `body`, so `__body__` carries the request body the turn was started with, as it stands after the `inlet()` filters ran.

### `__features__`

Tools only. The `features` dict from the request (`web_search`, `image_generation`, `code_interpreter` and any other toggles the client sent), taken from the body after the `inlet()` filters ran, so a Tool can see which toggles the user switched on.
