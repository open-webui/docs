...
When `allowSameOrigin` is **on** (via the user setting `iframeSandboxAllowSameOrigin`), the parent page can directly measure the iframe's content height and resize it automatically, with no script needed in your HTML. However, this comes with security trade-offs (see below).

## Sandbox and Security

Embedded iframes run inside a [sandbox](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/iframe#sandbox). `allow-scripts` and `allow-downloads` are always on. `allow-popups` is on for tool call results, for embeds inside a grouped details block and for embeds attached to an assistant message, but not for embeds opened from a citation in the Embeds panel.

Two more flags can be toggled by the user in **Settings → Interface**:

| Setting | Default | Description |
|---|---|---|
| iframe Sandbox Allow Same Origin | ❌ Off | Allows the iframe to access parent page context |
| iframe Sandbox Allow Forms | ❌ Off | Allows form submissions within embedded content |

Two caveats on those toggles. Embeds attached to an assistant message always allow forms regardless of the setting, and embeds opened from a citation ignore both toggles entirely, so their sandbox is only `allow-scripts allow-downloads`.
...
Users can toggle this setting in **Settings → Interface → iframe Sandbox Allow Same Origin**.
...
If your Rich UI embed needs to trigger downloads, interact with Open WebUI's frontend, or execute JavaScript that impacts the parent page, **enabling same-origin iframe access is required**. Enable it in **Settings → Interface → iframe Sandbox Allow Same Origin**.
...
If you want to see how far Rich UI can go when same-origin is enabled, take a look at the community **[Inline Visualizer v2](https://github.com/Classic298/open-webui-plugins)** tool (see the [Show-and-tell discussion](https://github.com/open-webui/open-webui/discussions/23901)).
...
When `allowSameOrigin` is enabled, the iframe component auto-detects usage of certain libraries in your HTML and injects them automatically, with no CDN `<script>` tags needed:

- **Alpine.js**: Detected when any `x-data`, `x-init`, `x-show`, `x-bind`, `x-on`, `x-text`, `x-html`, `x-model`, `x-modelable`, `x-for`, `x-if`, `x-effect`, `x-ignore`, `x-transition`, `x-cloak`, `x-ref`, `x-teleport`, or `x-id` directives are found
...
- When `allowSameOrigin` is **on**, `input:prompt:submit` submits the prompt **immediately**, with no user interaction required.
