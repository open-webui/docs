...
:::warning
When encryption is enabled, `WEBUI_SECRET_KEY` must be a valid 44-character url-safe base64 Fernet key, and rotating it makes previously encrypted Valve values unreadable (they must be re-entered). See [`ENABLE_VALVE_ENCRYPTION`](/reference/env-configuration#enable_valve_encryption) for details.
:::
