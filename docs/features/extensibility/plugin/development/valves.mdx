...
The renderer picks the control in a fixed order. A `Literal` or enum field always renders as a select, a `bool` as a switch, a multiselect as its own control, and any other non-string type as a plain text box that ignores `input.type`. The `password` and `select` input types therefore only take effect on `str` fields.
...
For options that need to be generated at runtime (e.g., fetching available models, databases, or user-specific resources), specify a method name as a string. The method is called synchronously each time the valves dialog opens, and it must return a list. An `async def` returns a coroutine, which fails that check and leaves the field with no options and a warning in the log. The method may take a `__user__` or `user` argument to receive the current user as a dict:
