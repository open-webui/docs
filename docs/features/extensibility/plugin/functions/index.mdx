...
Both are defined as nested Pydantic `BaseModel` classes inside your `Pipe`, `Filter`, or `Action` class. The admin-configured values are stored in the database and loaded automatically at runtime. When Valves hold secrets, an administrator can [encrypt them at rest](/features/extensibility/plugin/development/valves#encrypting-valve-values-at-rest).
