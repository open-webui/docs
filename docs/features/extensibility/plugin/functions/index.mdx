...
Open WebUI supports four types of Functions. The type is **auto-detected** from the class name in your Python code, so you never need to configure it manually.
...
| **Event** | `class Event` | Runs custom logic in response to system events | Runs in the background; no direct UI |
...
A single Pipe can also expose **multiple models** (called a "manifold") by defining a `pipes()` method that returns a list of `{"id": ..., "name": ...}` dictionaries, one per model.
...
*New in Open WebUI 0.10.0.*

An Event function runs custom Python in response to **system events** (a user signs up, a chat is deleted, the server starts, configuration changes). Its `event()` method is called for every event and filters on the event name. Event functions can register their own API endpoints and call internal services, which suits them to lifecycle automation, audit logging, signup gating, provisioning, and self-installing plugins.

→ **[Event Function Guide](./event)**
...
When you save a Function, Open WebUI scans the Python source code for a top-level class named `Pipe`, `Filter`, `Action`, or `Event`. The first match determines the function type. You never need to manually set the type because it's inferred from the code.
...
The Python source code is loaded via `exec()` into a temporary module namespace. Once loaded, the module is cached in memory (`request.app.state.FUNCTIONS`) and only reloaded when the source code changes. Four legacy prefixes are rewritten on the way in, `from utils`, `from apps`, `from main` and `from config`, each gaining an `open_webui.` prefix. The rewrite is a plain text replacement over the whole file, so those strings are also changed inside docstrings and prompt text, and the rewritten source is saved back to the database.

### When a Function Fails to Load

If the code raises while it is being executed, Open WebUI switches the function to **inactive** rather than leaving it to fail on every request. That covers a syntax error, an `import` that fails, an exception raised while the class is constructed and a file with no `Pipe`, `Filter`, `Action` or `Event` class in it. A failed `requirements` install is separate: it rejects the save and leaves the function's state alone.

- **Saving over an existing function** executes the code before storing it. The save is rejected and your new source is never written, so what ends up deactivated is the version already in the database. The browser only shows a generic error, the Python error goes to the server log as `Error loading module: <function id>: <the error>`.
- **Outside the editor**, the same execution happens every time the module is loaded again, so a function can turn itself off with nobody touching it in the admin panel. A package that was installed into the container by hand and never added to `requirements` is the usual cause, since it is gone after a restart.

Fix the code, then switch the function back on in **Admin Panel > Functions**.
...
When `requirements` is specified in the frontmatter, Open WebUI automatically installs the listed packages via `pip` the first time the function loads. This is controlled by the [`ENABLE_PIP_INSTALL_FRONTMATTER_REQUIREMENTS`](/reference/env-configuration#enable_pip_install_frontmatter_requirements) environment variable (enabled by default).
:::
...
2. Click **Import From Link** and paste a link to the Python file.
...
The globe selector in the function editor stores a name, a description, valve labels and per-action labels for each language, so an action button under a reply can read in the user's own language. See [Translations](/features/administration/translations).
...
Both are defined as nested Pydantic `BaseModel` classes inside your `Pipe`, `Filter` or `Action` class. An `Event` class can carry `Valves` too, but never `UserValves`, since event handlers run without a user. The admin-configured values are stored in the database and loaded automatically at runtime. When Valves hold secrets, an administrator can [encrypt them at rest](/features/extensibility/plugin/development/valves#encrypting-valve-values-at-rest).
...
| [Event Function Guide](./event) | Run custom logic in response to system events |
