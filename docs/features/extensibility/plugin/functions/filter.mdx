...
Raising from `request` aborts the whole completion, exactly as it does from `outlet` and `stream`. Only `inlet` failures are logged at debug level and otherwise treated gently; every other filter type re-raises and the call site turns it into a failed turn. If your filter can fail on malformed input and you would rather degrade than kill the answer, catch it yourself and return the body unchanged.
...
| `request()` | ✅ Called before every provider call | ✅ Called before every provider call | ✅ Called before every provider call |
...
**On `dev` / upcoming release:** `outlet()` runs for direct `/api/chat/completions` callers **by default**, on **both** the non-streaming and streaming paths, controlled by [`ENABLE_API_OUTLET_FILTERS`](/reference/env-configuration#enable_api_outlet_filters) (default `True`). On the streaming path the server accumulates the full response and runs `outlet()` once the stream completes. Set `ENABLE_API_OUTLET_FILTERS=False` to skip outlet filters on direct API traffic.
...
`request()` is handed the **same set** as `inlet()`, so `__model__` resolves workspace models there exactly as shown above. The one extra is `__features__`, which carries the `features` dict after the pipeline has popped it off the body.
