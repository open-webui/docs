...
2. **Modify Every Outgoing Call (Request Function)**: Adjust the payload immediately before it is handed to the model provider, every single time one is sent. Added in v0.11.2, and covered in [The `request` step](#the-request-step) below.
3. **Intercept Model Outputs (Stream Function)**: Capture and adjust the AI’s responses **as they’re generated** by the model. This is useful for real-time modifications, like filtering out sensitive information or formatting the output for better readability.
4. **Modify Model Outputs (Outlet Function)**: Adjust the AI's response **after it’s processed**, before showing it to the user. This can help refine, log, or adapt the data for a cleaner user experience.
...
class Filter:
    ...
    async def request(self, body: dict) -> dict:
        # This is where you manipulate the payload for each outgoing model call.
        print(f"request called: {body}")
        return body
...
`request` was added in **v0.11.2**. It receives the payload that is about to be sent to the model provider, and whatever it returns is what gets sent.

```python
    async def request(self, body: dict, __user__: Optional[dict] = None) -> dict:
        body["metadata"] = {**body.get("metadata", {}), "department": "research"}
        return body
```

#### How it differs from `inlet`

`inlet` runs **once per user turn**. `request` runs **once per call to the model**, and those are not the same number.

A turn that uses tools makes several calls: one to decide which tool to use, then another after each set of tool results comes back, until the model produces a final answer. `inlet` saw only the first of those. `request` runs before every one of them, including the call made after a tool run the user had to approve.

That difference decides which one you want:

| Use `inlet` when | Use `request` when |
| :--- | :--- |
| You are reacting to what the user typed | You are enforcing something on the wire |
| Running your logic more than once per turn would be wrong, such as billing, rate limiting or logging a turn | Running it on every call is the point, such as stamping a parameter, injecting a header or redacting content |
| You want to change the user's message | You want to change the request, whether the messages in it came from the user, the model or a tool |

Something added in `inlet` reaches the first call and can be dropped from later ones as the message list is rebuilt for each follow-up. Something added in `request` is on every call by construction, which is why enforcement belongs here.

#### Where `request` sits in the pipeline

`inlet` runs near the start of the turn; `request` runs at the end, on the payload as it is about to leave. Everything the pipeline does happens between them, and that changes what is in the `body` you receive.

**Gone by the time `request` runs.** The pipeline pops several keys out of the body after `inlet` returns: `features`, `tool_ids`, `terminal_id`, `files`, `folder_id` and `skill_ids`. A filter that needs any of these has to be an `inlet`. `variables`, `regeneration_prompt` and `params` are popped even earlier, before `inlet` runs, and `variables` stays readable as `__metadata__["variables"]`. (`features` is still reachable in `request` through the `__features__` reserved argument, which the pipeline sets aside before popping the key; the others are not.)

**Only present by the time `request` runs.** Retrieved RAG chunks wrapped in `<source>` tags, the resolved `tools` array (server-side tools merged with anything a filter or the caller added), the chat-controls system message and the folder system prompt (the workspace model's own System Prompt is added by the provider router after `request` returns), and memory or web-search context. If you want to read or rewrite the *retrieved chunk text* rather than the file references, `request` sees it natively, with no `file_handler` opt-in and no retrieval code of your own. That is often simpler than [owning retrieval](#owning-retrieval-with-file_handler); reach for `file_handler` when you need to *replace* how retrieval happens, and for `request` when you only need to transform its result.

Provider-shape normalization happens after `request` returns, so you still see Open WebUI's internal message shapes, not the provider's.

#### Write it to be idempotent

This is the one that bites. Because `request` runs again on every hop of a tool loop, and each hop passes the **whole conversation** back through your filter, any change you make must be safe to apply to a body you have already changed.

```python
# WRONG: a three-tool-call turn ends with four copies of this line.
async def request(self, body: dict) -> dict:
    body["messages"].insert(0, {"role": "system", "content": "Be concise."})
    return body

# RIGHT: converges to the same body no matter how many times it runs.
async def request(self, body: dict) -> dict:
    marker = "Be concise."
    if not any(m.get("content") == marker for m in body["messages"]):
        body["messages"].insert(0, {"role": "system", "content": marker})
    return body
```

Transformations that are naturally idempotent (clamping a value, downscaling an image that is already under the limit, redacting a pattern that no longer matches) need no guard. Anything that appends, prepends, increments or accumulates does. If a once-per-turn guarantee is what you actually want, that is what `inlet` is for.

#### Other things to watch for

`request` runs on every provider call, so anything expensive in it is paid for repeatedly inside a single answer. Because each call re-scans the full history, cost grows with both conversation length and tool-call count. Cache on a content hash if the work is heavy.

What a raised exception does depends on the hook. From `inlet`, or from the first `request` call of a turn, it aborts the turn. From `request` on a later tool-loop hop it ends the loop with an error while what was already streamed stays. From `stream` it drops that one event in the WebUI path, but ends the whole stream for a direct API streaming caller. From `outlet` it is logged at debug level and otherwise ignored, except through `/api/chat/completed`, where it becomes an HTTP 400. If your filter can fail on malformed input and you would rather degrade than kill the answer, catch it yourself and return the body unchanged.

`file_handler` has no effect on a `request` filter; see [the note below](#owning-retrieval-with-file_handler).

`inlet`, `stream` and `outlet` are unchanged, and a filter that does not define `request` behaves exactly as before.
...
- **Visibility:** the filter only appears in the chat UI (inline chip + Integrations menu entry) when `self.toggle = True` **and** it is either a global filter or attached to the selected model. Without `self.toggle`, the filter still runs (if active and in scope) but has no UI surface, so users can't turn it off.
- **Gating:** at request time the backend checks the user's current `filter_ids` selection. If the filter is in that list, `inlet()` / `request()` / `stream()` / `outlet()` run. If not, the filter is **not invoked at all**.
...
- Initial state comes from the model's `defaultFilterIds` (Settings > Admin > Models → Edit the model → Default Filters). Admins decide which toggleable filters start **on** vs **off** per model.
...
class Filter:
    ...
    def __init__(self):
        self.valves = self.Valves()
        # Read from the instance before each inlet() call.
...
:::warning Instance attribute, not a module-level variable
Open WebUI reads `file_handler` from the `Filter` instance, the same way it reads `toggle` and `icon`. A bare `file_handler = True` at the top of the file is silently ignored. Set `self.file_handler = True` in `__init__`, or declare it as a class attribute, exactly as shown above.
:::

:::note `file_handler` applies to `inlet` only
The flag is read exclusively on the `inlet` pass, and the file references are stripped only after `inlet` filters have run. Declaring `file_handler = True` in a module whose filter defines only `request` (or `stream` / `outlet`) does nothing at all: the built-in RAG still runs, and no keys are stripped. If your goal is to transform retrieved chunks rather than replace retrieval, you do not need this flag; a `request` filter already receives the chunks after injection. See [Where `request` sits in the pipeline](#where-request-sits-in-the-pipeline).
:::
...
`file_handler` is read from the cached `Filter` instance before each `inlet()` call. It is not a per-request signal: a value changed inside `inlet()` applies from the next request, not the one being processed, and it cannot be flipped based on the model, user, or chat of the current request. When set, the built-in RAG is **always** skipped for any request where this filter is invoked, regardless of whether your `inlet()` actually called any retrieval logic on that particular request.
...
5. Flip the **Global** switch in that menu
6. Ensure the filter is also **Active** (green toggle switch)

**API Endpoint:**
```http
POST /api/v1/functions/id/{filter_id}/toggle/global
```

**Visual Indicators:**
- 🟢 Green toggle = `is_active=True` (filter is active)
- The list itself does not show whether a filter is global. Open the ⋮ menu to read the **Global** switch.
...
**5. FILTER EXECUTION**
- Execute inlet() filters (once, at the start of the turn)
- Execute request() filters (immediately before the payload goes to the provider)
- Send modified request to LLM
- Execute stream() filters (during streaming)
- Repeat request() and stream() for every further call the turn makes, such as after tool results come back
- Execute outlet() filters (once, after the final response)

---

### Filter Behavior with API Requests

When using Open WebUI's API endpoints directly (e.g., via `curl` or external applications), `inlet()`, `request()` and `stream()` follow the same execution model as WebUI requests. `outlet()` is the one that behaves very differently for direct API callers and is covered in detail below.

#### Key Behavioral Differences

| Function | WebUI Request | Direct API |
|----------|--------------|------------|
| `inlet()` | ✅ Always called | ✅ Always called |
| `request()` | ✅ Called before every provider call | ✅ Called before every provider call |
| `stream()` | ✅ Called during streaming | ✅ Called during streaming |
| `outlet()` | ✅ Called after response | ✅ Runs by default since v0.10.0, gated by `ENABLE_API_OUTLET_FILTERS` (see below) |
| `__event_emitter__` | ✅ Shows UI feedback | ⚠️ A real but invisible callable in `inlet()` and the first `request()`, and `None` in `stream()` and `outlet()` |

:::info outlet() on Direct API Calls
Since v0.10.0, `outlet()` runs for direct `/api/chat/completions` callers **by default**, on **both** the non-streaming and streaming paths, controlled by [`ENABLE_API_OUTLET_FILTERS`](/reference/env-configuration#enable_api_outlet_filters) (default `True`). On the streaming path the server accumulates the full response and runs `outlet()` once the stream completes. Set `ENABLE_API_OUTLET_FILTERS=False` to skip outlet filters on direct API traffic.

**Response visibility:** `outlet()`'s side effects run (DB update, `__event_emitter__`, external calls like logging/tracing), but it does **not** rewrite the HTTP response body, and a streaming client has already received the stream before `outlet()` fires. To observe `outlet()`'s output, read the chat record back, subscribe to the `chat:outlet` WebSocket event, or use `/api/chat/completed`. `__event_emitter__` is `None` in `outlet()` for pure API callers.
:::

#### Inlet ↔ Outlet Correlation via `__metadata__`

`__metadata__` is a live dict passed through the request lifecycle, so anything your filter stashes in `inlet()` is visible in `outlet()` on the *same request*. This is useful for things like start-time tracking, correlation IDs, or per-call valves, *when `outlet()` actually runs*. That covers WebUI requests and the inline outlet for direct `/api/chat/completions` calls, where the same dict object flows from `inlet()` to `outlet()`. It does not cover `/api/chat/completed`, which builds a fresh metadata dict holding only `chat_id`, `message_id`, `filter_ids`, `session_id` and `user_id`.
...
Older versions of this page suggested synthesizing a temporary-chat `chat_id` (`temporary:<uuid>`, or `local:<uuid>` in older releases) and a random `message_id` inside `inlet()` to force `outlet()` to run for pure API callers. Do not rely on that pattern. In current backend code the chat ownership check runs before the filter pipeline, and the inline outlet handler does not rewrite the HTTP response body even when it does run. If you need `outlet()` output over HTTP for an API caller, use `/api/chat/completed` instead.
:::

#### Running outlet() for API Callers: `/api/chat/completed`

The reliable, supported way to run `outlet()` for a direct API integration is to follow up `/api/chat/completions` with `POST /api/chat/completed`, passing the full conversation (including the assistant response) in `messages`. The body must also carry `id`, the assistant message id, and the `chat_id` and `session_id` keys (their values may be empty, but a non-empty `chat_id` must be a chat you own). The endpoint runs pipeline outlet filters and Function `outlet()` handlers unconditionally and returns the filtered payload. Errors come back as HTTP 400.
...
curl -X POST http://localhost:3000/api/chat/completed \
  ...
  -d '{
    ...
    "id": "the-assistant-message-id",
    "chat_id": "a-chat-id-you-own",
    "session_id": ""
  }'
```

:::info
This endpoint is labeled deprecated in favor of inline execution, but because inline execution does not return the filtered payload over HTTP to pure API callers, `/api/chat/completed` is still the correct choice for most API integrations today.
:::
...
Filters that use `__event_emitter__` will still execute for API requests, but since there's no WebUI to display the events, the status messages won't be visible. In `inlet()` and the first `request()` the emitter is a real callable, so awaiting it is harmless. In `stream()` and `outlet()` it is `None` for pure API callers, so guard with `if __event_emitter__:` before awaiting it.
:::
...
Filters can only modify the **request body** (`form_data`). Outbound HTTP headers are constructed separately and cannot be influenced from a filter. To add custom headers to API requests, use the **Settings → Admin → Connections → OpenAI API** headers configuration.
:::
...
| `__user__` | The full user record (`UserModel.model_dump()`), with `valves` added when the filter defines `UserValves` |
...
| `__event_call__` | Function that sends an event and waits for the browser's answer |
| `__chat_id__` | Chat session ID. Not passed to `outlet()`, which reads it from `body["chat_id"]` |
| `__message_id__` | Id of the assistant message being produced. Not passed to `outlet()` |
| `__oauth_token__` | The user's OAuth token data, or `None`. Not passed to `outlet()` |
| `__id__` | The filter's own function id |
| `__body__` | `stream()` only: the request body being answered, since `stream()` receives the chunk as `event` |
...
`request()` is handed the same set as `inlet()` on the first call of a turn, so `__model__` resolves workspace models there exactly as shown above, plus `__features__`, which carries the `features` dict after the pipeline has popped it off the body. On the later tool-loop hops `__features__` is absent, so give it a default.
...
| **Global** switch in the ⋮ menu | Filter is global (`is_global=True`) when on |
...
| Toggle switch | Enable/disable the filter for this chat, and the only indicator of its state |
...
At `inlet()` time, `body["metadata"]["files"]` and `body["files"]` contain only file/collection *references*. The actual chunk text is fetched and injected later, after every inlet filter has returned. Two ways to reach the chunk text itself (PII redaction, reranking, translation, chunk-level ACLs):

- **[`request()`](#the-request-step)** runs after retrieval, so the injected chunks are already in the body. Use this to *transform* what the built-in RAG produced. Simplest option, no opt-in required.
- **[`file_handler`](#owning-retrieval-with-file_handler)** skips the built-in RAG entirely and hands you the references. Use this to *replace* retrieval with your own.
:::
...
##### Streamed Events on Responses API Connections

The events above are **Chat Completions** chunks, which is what you get from everything except a connection whose **API Type** is set to `Responses`. Those forward the provider's own event stream unchanged, so `stream()` receives typed Responses API events instead. The stream opens with bookkeeping events that carry no text, so trimmed to the ones that do:

```jsonl
{"type": "response.output_text.delta", "delta": "Hi"}
{"type": "response.output_text.delta", "delta": " 😊"}
{"type": "response.output_text.done", "text": "Hi 😊"}
{"type": "response.content_part.done", "part": {"type": "output_text", "text": "Hi 😊"}}
{"type": "response.output_item.done", "item": {"type": "message", "content": [{"type": "output_text", "text": "Hi 😊"}]}}
{"type": "response.completed", "response": {"output": [{"type": "message", "content": [{"type": "output_text", "text": "Hi 😊"}]}], "usage": {"input_tokens": 9, "output_tokens": 3}}}
```

:::warning Rewriting the deltas alone is not enough
The deltas are what the user watches arrive, but the later events repeat the **full text**, and Open WebUI rebuilds the message from them and pushes the rebuild to the browser. An edit applied only to `response.output_text.delta` is undone on screen the moment `response.output_text.done` arrives; `response.content_part.done` then replaces the whole content part, `response.output_item.done` the whole output item and `response.completed` the whole response with the provider's copy, which is what gets saved when the response ends. The reverse is true too: some providers close with an empty `output`, in which case Open WebUI keeps what the earlier events built, and an edit made only in `response.completed` never reaches the saved message. Edit every event that carries the text.

The example below covers the assistant message text. Reasoning is a separate output item type with text of its own, and refusals sit in a `refusal` field rather than a `text` one, so neither is touched by it.
:::

##### Example: Filtering Out Emojis on Responses API Connections

```python
async def stream(self, event: dict) -> dict:
    def clean(value):
        # Non-string values turn up, and raising here would drop the whole event
    ...
    for choice in event.get("choices") or []:
        ...
        if "content" in delta:
            delta["content"] = clean(delta["content"])

    # Responses API events
    event_type = event.get("type", "")
    if event_type == "response.output_text.delta" and "delta" in event:
        event["delta"] = clean(event["delta"])
    elif event_type == "response.output_text.done" and "text" in event:
        event["text"] = clean(event["text"])
    elif event_type == "response.content_part.done":
        part = event.get("part") or {}
        if "text" in part:
            part["text"] = clean(part["text"])
    elif event_type == "response.output_item.done":
        clean_message(event.get("item"))
...
If `stream()` raises, Open WebUI drops that whole event, and losing `response.completed` costs the reply its token usage figures and its response id.

:::note Valves are resolved once per response, not once per chunk

`stream()` runs for every fragment, so its valves are read from the database once when the response starts and reused for the rest of it, rather than being re-read thousands of times. Both `self.valves` and `__user__["valves"]` are therefore stable for the whole of one streamed reply: a valve edited while a reply is streaming takes effect on the next reply, not partway through the current one. `inlet()` and `outlet()` are unaffected, as each runs once anyway.
...
On tagged releases `outlet()` is never invoked by `/api/chat/completions`, so a direct API integration has to follow it with `POST /api/chat/completed`. On `dev` it runs by default for direct callers on both the streaming and non-streaming paths, controlled by [`ENABLE_API_OUTLET_FILTERS`](/reference/env-configuration#enable_api_outlet_filters). Either way the filtered content is not written back into the HTTP response. See [Filter Behavior with API Requests](#filter-behavior-with-api-requests) for the full picture.
:::
...
1. **Inlet** manipulates **user inputs** (pre-processing), once per turn.
2. **Request** adjusts the **outgoing payload** before every provider call, including each hop of a tool loop. Write it to be idempotent.
3. **Stream** intercepts and modifies **streamed model outputs** (real-time).
4. **Outlet** tweaks **AI outputs** (post-processing).
5. Filters are best for lightweight, real-time alterations to the data flow.
6. With **Valves**, you empower users to configure Filters dynamically for tailored behavior.
