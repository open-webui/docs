...
- Initial state comes from the model's `defaultFilterIds` (Settings > Admin > AI > Models → Model Settings → Default Filters). Admins decide which toggleable filters start **on** vs **off** per model.
