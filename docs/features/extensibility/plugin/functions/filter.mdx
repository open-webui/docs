...
When using Open WebUI's API endpoints directly (e.g., via `curl` or external applications), filters now follow the same execution model as WebUI requests.
...
| `outlet()` | ✅ Called after response | ✅ Called after response |
| `__event_emitter__` | ✅ Shows UI feedback | ⚠️ Runs but no UI to display |

:::info Outlet Runs Inline
`outlet()` now runs inline as part of `/api/chat/completions` processing for both WebUI and direct API requests.
...
`outlet()` is called for direct API requests to `/api/chat/completions`.
:::
