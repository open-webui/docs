...
`request` runs on every provider call, so anything expensive in it is paid for repeatedly inside a single answer. Keep it cheap, and keep anything that should happen once per turn in `inlet`.
...
:::info How assistant text is stored (read this before editing)
Assistant messages carry **two** representations of their text:

- **`content`** — a plain string. Backend consumers read this: memory extraction, RAG/retrieval context, context-window compaction, and follow-up generation.
- **`output`** — a structured list of items (reasoning blocks + `message` items). **This is what the chat UI renders.** The visible response text lives in `output[].content[].text` for items where `type == "message"`.

Because the UI reads `output` while backend consumers read `content`, editing **only** `message["content"]` updates the stored/derived text but **not** what the user sees on screen (and vice-versa). To make a change appear in the UI *and* stay consistent for the rest of the backend, rewrite **both** representations together:

```python
def redact(message: dict, old: str, new: str) -> None:
    """Rewrite both the flat `content` and the structured `output` in place."""
    # 1) Flat content — read by backend consumers (memory, RAG, context, follow-ups).
    if isinstance(message.get("content"), str):
        message["content"] = message["content"].replace(old, new)

    # 2) Structured output — what the chat UI actually renders.
...
> Scope the `output` loop to `item.get("type") == "message"` if you only want to touch the visible answer and leave the (often hidden) reasoning trace alone.
:::
...
async def outlet(self, body: dict, __user__: Optional[dict] = None) -> dict:
    for message in body["messages"]:
        redact(message, "<API_KEY>", "[REDACTED]")  # `redact` defined above
    return body
```
...
class Filter:
    async def outlet(self, body: dict, __user__: Optional[dict] = None) -> dict:
        # Add "highlight" markdown for every assistant response.
        # Keep the flat `content` and the structured `output` in sync —
        # the UI renders from `output`, the backend reads `content`.
        for message in body["messages"]:
            if message.get("role") != "assistant":  # Target model response only
                continue
            if isinstance(message.get("content"), str):
            ...
            for item in message.get("output") or []:
                ...
                for part in item.get("content") or []:
                    if isinstance(part.get("text"), str):
                        part["text"] = f"**{part['text']}**"  # Highlight with Markdown
        return body
```
