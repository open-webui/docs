...
3. The request is **non-streaming**. On the streaming path the server consumes the stream itself and routes content to the user's WebSocket, so an HTTP streaming API caller receives effectively nothing. `outlet()` still runs DB-side, but the streaming client does not see it.
