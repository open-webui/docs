...
- **Settings** → **Admin** → **AI** → **Models** → select a model → **Advanced Parameters**: alternative per-model settings location
...
This Ollama-specific setting controls the model's built-in reasoning feature. Ollama has separated thinking from the answer natively since v0.9.0, so there is no server-side flag to set:

| Option | What is sent | Description |
|--------|--------------|-------------|
| **Default** | nothing | Uses Ollama's default behavior, which is thinking on for models that support it |
| **On** | `think: true` | Explicitly enables thinking mode for the model |
| **Custom** | `think: "low"`, `"medium"`, `"high"` or `"max"` | Sets a thinking level. GPT-OSS accepts only a level, never `true` |
| **Off** | `think: false` | Disables thinking mode |

:::note
This setting sends the `think` parameter directly to Ollama, which returns the trace in a separate `thinking` field that Open WebUI reads as reasoning (see [Path 2](#path-2-reasoning-captured-into-a-structured-output-array) below). It's separate from how Open WebUI parses `<think>` tags in text, which only matters for models that write their reasoning inline.
:::
...
:::info
Reasoning Effort is only applicable to models from specific providers that support this parameter. It is not forwarded on native Ollama connections, where the `think` setting above covers the same ground. Ollama's OpenAI-compatible endpoint does accept `reasoning_effort` when Ollama is added as an OpenAI connection instead.
:::
...
| Every other option (`Default`, `Azure OpenAI`, `LM Studio`, `LiteLLM`) | Reasoning is **omitted** from the rebuilt message |
...
| :--- | :--- | :--- |
| OpenAI-compatible APIs that emit `<think>` tags inline (a llama.cpp or vLLM server not configured to split reasoning out, for example) | ✅ Round-trips automatically. | n/a |
...
- **Most users are on Path 1.** Models that emit `<think>` tags inline (the DeepSeek-R1 family on Ollama, Qwen3, MiniMax M3, etc.) keep their reasoning in `content` and continue to round-trip without any provider configuration.
...
Providers that send reasoning metadata alongside the answer rather than before it are handled too: reasoning that arrives after the response has started is folded into the thinking block above the answer instead of appearing beneath it, and metadata carrying no actual reasoning text no longer opens an empty thinking block.
...
2. The `<think>` content is only separated if the tags are detected in the finished text
3. The raw response is displayed as-is

:::info Important
If you're using non-streaming requests (via API or certain configurations), **thinking block separation depends on the tags being detected in the full response**, or on the backend returning the reasoning as its own field.
:::
...
Ollama does not take a parser flag. `ollama serve` has no command-line options, and the `--reasoning-parser` flag some guides mention belongs to vLLM. Ollama splits thinking from the answer itself once the `think` parameter is on, and Open WebUI shows that split without any server-side setup.

#### Step 1: Check the Ollama Version

Thinking support arrived in Ollama v0.9.0. Check what you are running:

```bash
ollama --version
```

#### Step 2: Turn Thinking On for the Model

Open **Chat Controls > Advanced Params** (or the model's **Advanced Params** in the model editor) and set **think (Ollama)** to **On**, or to **Custom** with a level (`low`, `medium`, `high` or `max`) for models such as GPT-OSS that accept only a level. **Default** leaves the choice to Ollama, which turns thinking on for supported models.
...
3. The response should now show the thinking content in a collapsible section above the answer

If the model writes its reasoning inline as `<think>...</think>` text instead of using Ollama's thinking field, check the **Reasoning Tags** setting above, and use **Custom** for any other tag pair.

### Troubleshooting Checklist

#### 1. Verify Ollama Separates the Thinking

Run the model from the Ollama CLI with thinking on. A supported model prints the trace and the answer as two parts:

```bash
ollama run your-model:tag --think "Explain step by step: What is 15 + 27?"
```

If Ollama says the model does not support thinking, the model has no thinking mode, and Open WebUI can only pick up tags it writes into the text.
...
### Stateful Reasoning Models (GPT-5.6, etc.)

**Symptom:** Using a model that hides its reasoning (stateful/internal reasoning), and reasoning is not being preserved.

**Cause:** Some newer models (like GPT-5.6) keep their reasoning internal and don't expose it in the API response. Open WebUI can only preserve reasoning that is actually returned by the model.
