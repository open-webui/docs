...
Automations can be enabled or disabled globally by an administrator via the [`ENABLE_AUTOMATIONS`](/reference/env-configuration#enable_automations) environment variable or the **Settings > Admin > General** toggle. When disabled, the automation worker stops processing, API endpoints return `403`, and the sidebar entry is hidden.
...
- Give runs a terminal by configuring one on the model the automation uses
...
### Giving a Run a Terminal

The automation editor has no terminal setting. A run uses the terminal configured on the model it runs, so set that up first in **Workspace > Models**: edit the model and pick the terminal from the **Terminal** list. The list appears when at least one terminal is available to you and the model's **Terminal** capability is on, which it is by default. Every automation using that model then runs with that terminal.

:::info A terminal can be closed to automations
Administrators can take an orchestrated terminal out of automations, and can give each automation a workspace of its own rather than sharing one across everything. The model editor's **Terminal** list does not reflect that, so a run can fail with an error saying the terminal is not available for automations, which means its **Automation** context is set to **Off**. Pick a different terminal for the model, or ask an administrator to reopen that one. See [Terminal Contexts](/features/open-terminal/terminals/orchestration/contexts).
:::
...
- Update title, instructions, model and schedule
- Save changes
...
- The automations list can be narrowed to a folder through the API and the chat tools; the list page itself does not label or filter by folder. Asking for the list without a folder, or with an empty one, returns every automation you have, whichever folder each is filed into.
...
The folder argument these tools accept is a folder ID, and it is checked against the folders you own: a folder that is not yours, or no longer exists, comes back as an error instead of being applied.

- On `update_automation`, omitting the argument or passing a blank string keeps the automation's current folder. Passing `null` clears it. The same applies to `model_id`: a blank value is ignored rather than wiping the automation's model.
- On `list_automations`, omitting the argument or passing a blank string lists across all folders.
