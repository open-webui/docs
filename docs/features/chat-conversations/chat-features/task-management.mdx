...
Because the list is stored on the chat itself, both tools need a saved chat. In a [temporary chat](/features/chat-conversations/chat-features/url-params#8-temporary-chat-sessions) or a [channel](/features/channels) there is no chat record to write to, so the tools are not offered to the model at all and it plans in plain text instead.
