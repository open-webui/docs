...
One gap to know about: **your own direct tool-server connections (Settings > Tools) are not carried across**. They are not part of what a timer captures, so a fired timer runs without them even if the chat had them attached. Admin-configured tool servers travel with the chat's tool ids and do run. The captured set also includes the attached files, the terminal connection and the prompt variables. Keep that in mind for a prompt whose whole job depends on an external tool server.
