...
- **Switching chats in the same tab** - The queue stays in memory, keyed by chat id, and is there when you come back
- **Returning to the chat** - Queue is restored and processed
- **Reloading the page, opening a new tab, or closing the tab** - The queue is cleared, since it lives in memory only
