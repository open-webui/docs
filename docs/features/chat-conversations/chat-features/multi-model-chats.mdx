...
*   **Task Model**: The specific model used to perform the merger can be configured in **Settings > Admin > Experience > Interface > Tasks**. We recommend using a capable model (such as GPT-5.6 Sol or Claude Sonnet 5) as the task model for the best results.
