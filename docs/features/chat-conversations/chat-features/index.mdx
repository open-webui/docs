...
- **Built-in Slash Commands**: these act on the chat or the interface rather than being sent to the model:
  ...
  | `/skills:create` | Turns the conversation into a reusable [skill](/features/workspace/skills#from-a-chat-with-skillscreate), saved on the selected terminal. Offered only when an Open Terminal is selected and the chat already has content, and anything you type after it steers what the skill covers. |
  | `/temporary` | Turns temporary chat on or off for the chat you are about to start. The menu entry shows the current state beside it, **On** or **Off**. |
...
- **🔗 Link Rendering**: Links in responses, citations and web search results become clickable only for `http`, `https`, `mailto`, `tel` and relative URLs. Any other scheme, `ftp:`, `sms:` and application deep links such as `obsidian://` or `vscode://`, renders as plain text with its label intact, so a model cannot hand you a one-click jump into a local application.
