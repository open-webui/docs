...
The server still runs the rest of the turn, including background steps such as the chat title and tags, and those steps read the instance's model list. They read one consistent copy of it, so on a Redis-backed multi-replica deployment a refresh of that list landing mid-request no longer fails the message, and a long model list no longer slows the turn down ([details](/troubleshooting/multi-replica#3-model-not-found-or-configuration-mismatch)).
...
1.  **Admin Enablement**: The administrator must enable this feature globally.
    *   **Settings > Admin > Connections > Direct Connections**: Toggle **On**.
...
:::warning Security Recommendation: Provider Key Scope
Use least-privilege provider credentials for Direct Connections.

If your provider supports scoped keys, prefer inference-only keys for regular usage. Avoid using management/master keys for day-to-day user traffic.
:::
