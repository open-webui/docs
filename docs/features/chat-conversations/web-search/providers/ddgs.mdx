...
2.  Select **DDGS** as the **Web Search Engine**. The engine value behind that entry is `duckduckgo`, so set `WEB_SEARCH_ENGINE=duckduckgo` when configuring by environment variable. `ddgs` matches nothing. The entry is disabled on the slim image, where DDGS is unavailable.
