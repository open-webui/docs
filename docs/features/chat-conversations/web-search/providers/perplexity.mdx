...
5. Optionally set **Perplexity Model** (`PERPLEXITY_MODEL`, default `sonar`) and **Perplexity Search Context Usage** (`PERPLEXITY_SEARCH_CONTEXT_USAGE`, `low`, `medium` or `high`, default `medium`). The answer's citations become the results, titled `Source 1`, `Source 2` and so on, with the answer text as the first result's snippet.
6. Press save
