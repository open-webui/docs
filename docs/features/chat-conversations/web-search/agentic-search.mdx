...
Agentic web search needs a model that can call tools reliably and decide when to dig deeper. That does **not** mean a frontier model — cheap and fast is fine. **GPT-5.6 Luna**, **Gemini 3.5 Flash-Lite**, **MiniMax M3**, **DeepSeek V4 Flash**, or locally **Gemma 4 31B**, **Qwen 3.6 27B**, or **Muse Glimmer 30B** all handle this well and meet the minimum bar, each verified working at the time of writing. Only genuinely old or very small models struggle with the multi-step reasoning.
...
For comprehensive information about all built-in agentic tools (including web search, knowledge bases, memory, and more), see the [**Native/Agentic Mode Tools Guide**](/features/extensibility/plugin/tools#built-in-system-tools-nativeagentic-mode).
:::
...
To unlock these features, your model needs native tool calling and reasonable multi-step reasoning — at minimum something like GPT-5.6 Luna, Gemini 3.5 Flash-Lite, MiniMax M3, Gemma 4 31B, or Muse Glimmer 30B locally. Administrator-level configuration for these built-in system tools is handled via the [**Central Tool Calling Guide**](/features/extensibility/plugin/tools#tool-calling-modes-default-vs-native).

1.  **Enable Web Search Globally**: Ensure a search engine is configured in **Settings → Admin → Web Search**.
2.  **Enable Model Capability**: In **Settings → Admin → Models**, select your model and enable the **Web Search** capability.
...
5.  **Use a Quality Search Engine**: Choose a provider that returns relevant, current results. Hosted providers built for LLM workflows usually return stronger results, while self-hosted engines like SearXNG depend heavily on upstream engines and instance configuration.
6.  **Use a Quality Tool-Calling Model**: Use a recent model with reliable native function calling and enough context for tool schemas and search results.

:::info Configuring via environment variables instead
The steps above use the Admin Panel, which is the recommended route. If you manage your instance through env vars, set the global default with `DEFAULT_MODEL_PARAMS: '{"function_calling": "native"}'`. There is **no** separate `FUNCTION_CALLING_MODE` variable. See the [central tools guide](/features/extensibility/plugin/tools#how-to-enable-native-mode-agentic-mode) for details.
:::
...
Legacy Mode's RAG-style injection behavior is documented here only for legacy deployments. Native Mode is the default; Legacy Mode is the unsupported opt-out.
...
*   **Result Count Behavior**: If the model omits `count`, Open WebUI uses the admin-configured `WEB_SEARCH_RESULT_COUNT`. If the model provides `count`, the value is capped at the same admin maximum.
...
*   **Not a source**: search hits are not listed in the citations panel and the model is not told to cite them. Nothing has opened those pages, so there is nothing to cite. The results still reach the model as tool output and it can name a link in its answer like any other text.

:::info Changes in this release
Search results used to be emitted as citation sources, one empty entry per hit, all named `search_web`. Models regularly attached a result id to text taken from a different result, and the panel resolved that id to a title that looked right, so a wrong attribution was indistinguishable from a correct one. Search results are no longer offered as citations. To get a citation, the model has to open the page with `fetch_url`.

This applies to the Native tool-calling path. Legacy mode still runs the old RAG-style web search and still cites what it retrieves.
:::
...
*   **Citable**: a fetched page appears in the citations panel as a source named by its URL. This is the only way a web page becomes a citation in Native mode, and it is a page the model has actually read.
...
<details>
<summary>📖 Detailed Research Cycle Example</summary>
...
```
- Receives snippets based on the admin-configured `WEB_SEARCH_RESULT_COUNT` (or a smaller model-provided `count`)
...
For more details on how Interleaved Thinking works across all agentic tools (not just web search), see the [**Interleaved Thinking Guide**](/features/extensibility/plugin/tools#interleaved-thinking).
:::

## Next Steps

- **Save your findings**: Learn how to [save web search results directly to your Knowledge Base](./save-to-knowledge.mdx).
