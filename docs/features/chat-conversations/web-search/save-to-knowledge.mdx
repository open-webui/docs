...
:::warning Native mode lists fetched pages only
In Native (agentic) tool calling, search hits are not sources, so the only web entries this action can offer are pages the model opened with `fetch_url`. A reply where the model answered from snippets alone has nothing for it to save. Legacy tool calling is unchanged and still lists every retrieved search result. See [Agentic Search](/features/chat-conversations/web-search/agentic-search).
:::
...
- **No URLs Found**: This action reads the sources attached to the message. If no URLs are detected, check that web search is enabled and returning results, and in Native mode that the model actually opened a page with `fetch_url`, since search hits alone are not sources.
