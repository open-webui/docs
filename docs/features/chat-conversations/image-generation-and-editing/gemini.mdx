...
<Tabs>
  <TabItem value="gemini-3-pro" label="Gemini 3 Pro Image (aka Nano Banana Pro)" default>
...
- **API Key**: Enter your API Key
  ...
  <TabItem value="gemini-3.1-flash" label="Gemini 3.1 Flash Image (aka Nano Banana 2)">
...
Leaving the method empty falls back to `predict`, so a Gemini model such as `gemini-3-pro-image` fails until you switch it to `generateContent`.

`400: [ERROR: models/gemini-3-pro-image is not found for API version v1beta, or is not supported for predict. Call ListModels to see the list of available models and their supported methods.]`
