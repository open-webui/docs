...
3.  **Minimum Model Tier**: Use a current model with solid tool calling, at minimum GPT-5.6 Luna, Gemini 3.5 Flash-Lite, MiniMax M3, or Muse Glimmer 30B locally. Very small models may not manage memories reliably.
...
### Global Toggle
The Memory feature can be toggled on or off for the entire instance. When disabled, the "Personalization" tab is hidden from all users, the memory-related API endpoints are blocked, stored memories are no longer injected into the system context and the model is no longer given the memory tools.
...
- **Background review**: when [`ENABLE_MEMORY_BACKGROUND_REVIEW`](/reference/env-configuration#enable_memory_background_review) is enabled (default **off**), Open WebUI periodically reviews recent conversation with the chat's own model and can add, replace, move or remove memories of either type, `user` included, every [`MEMORIES_REVIEW_INTERVAL_TURNS`](/reference/env-configuration#memories_review_interval_turns) turns (default `10`), instead of relying only on explicit tool calls.
