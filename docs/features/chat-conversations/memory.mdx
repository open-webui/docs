...
2.  **Native Mode (Agentic Mode)**: Enable **Native Function Calling** in the model's advanced parameters (**Settings > Admin > Models > Edit** the model **> Advanced Params**).
...
- **Admin UI**: Settings > Admin > General > Features > **Memories**
...
Taking the permission away closes every route memories take to a model. The affected users lose the **Personalization** tab, the memory API endpoints answer them with `403`, the model is no longer handed the memory tools and their stored memories are no longer injected into the system context. That last check runs server-side on each chat request, so a request that still asks for memory (a personal **Memory** toggle left on from before, or an API client sending `features: {"memory": true}`) does not bring the memories back. Administrators are exempt from the permission check.
...
- **Admin UI**: Settings > Admin > General > **Memory System Context** (shown when Memories is enabled)
- **Environment Variable**: [`ENABLE_MEMORY_SYSTEM_CONTEXT`](/reference/env-configuration#enable_memory_system_context) (Default: `True`)

The injected block is wrapped in `<memory_context>` tags and appended to the system message. It always renders in the same order: the sections `[User Memory]`, `[Memory Neighborhood]` and `[Relevant Context]`, with the entries inside each section sorted alphabetically. The same set of memories therefore produces exactly the same text on every turn, so the model's cached prompt prefix survives instead of being thrown away because the vector search happened to return the same memories in a different order. See [Prompt Caching](/features/chat-conversations/prompt-caching#5-handle-memory-deliberately) for what still changes the block mid-conversation.
