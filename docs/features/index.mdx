...
- 🧠 **Memory Feature & Tools (Experimental)**: Manage information you want your LLMs to remember via `Settings` > `Personalization` > `Memory`. Capable models can now use `memory_add` and `memory_query` tools to dynamically store and retrieve facts about you during chat sessions. [Learn more about Memory](/features/memory).
