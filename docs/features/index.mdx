...
- 🖼️ **Custom Background Support**: Users set a custom background from Settings > Interface to personalize the experience. Administrators can configure a default background image with the DEFAULT_BACKGROUND_IMAGE environment variable or by setting a custom background from Admin Panel > Settings > Interface.
