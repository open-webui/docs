...
#### Example: Model Router + Context Manager Chain

A common use case is having one filter route messages to different models, followed by another filter that adjusts context based on the selected model.

:::note Example Only
The following examples are simplified to illustrate the concept. They show incomplete filter implementations—you would need to add the actual routing logic and helper methods for a working filter.
:::

**Filter 1: Model Router (priority=0)**
```python
class Filter:
    class Valves(BaseModel):
    ...
    async def inlet(self, body: dict, __event_emitter__) -> dict:
        # Example: Route complex tasks to a more capable model
        # You would implement is_complex_task() with your own logic
        if self.is_complex_task(body):
...
**Filter 2: Context Manager (priority=1)**
```python
class Filter:
    class Valves(BaseModel):
        priority: int = 1  # Runs after model router
    
    async def inlet(self, body: dict, __event_emitter__) -> dict:
        # Example: Trim messages based on the selected model's context window
        # You would implement get_context_limit() and trim_to_fit() with your own logic
        model_id = body.get("model")
        max_tokens = self.get_context_limit(model_id)
        body["messages"] = self.trim_to_fit(body["messages"], max_tokens)
        return body
```
...
User Input
    ↓
Model Router Filter (priority=0) → changes body["model"] to "claude"
    ↓
Context Manager Filter (priority=1) → receives body with model="claude" ✓
    ↓
...
### 🔀 Model Routing in Filters

Filters can dynamically route messages to different models by modifying `body["model"]` in the `inlet` function. To ensure the UI also displays the correct model in real-time, you need to emit a `chat:model` event using the event emitter.

#### Model Routing Example

:::note Example Only
This is a simplified example to demonstrate the model routing concept. The `requires_deep_reasoning()` method shown here is just placeholder logic—you would implement your own routing criteria based on your specific use case.
:::
...
class Filter:
    ...
    async def inlet(
        ...
        # Route based on task complexity (implement your own logic)
        if self.requires_deep_reasoning(user_message):
            new_model = self.valves.complex_model
        else:
            new_model = self.valves.simple_model
        
        # Only emit event if model actually changed
    ...
    def requires_deep_reasoning(self, text: str) -> bool:
        # Example logic - replace with your own criteria
        complex_indicators = ["analyze", "debug", "explain why", "step by step"]
        return any(indicator in text.lower() for indicator in complex_indicators)
```

**Key points:**
- Filter functions should generally be `async` to ensure compatibility with future Open WebUI versions as the backend progressively moves toward fully async execution. Synchronous filters may block execution in future releases.
- Include `__event_emitter__` in the function signature to emit events
- Emit `chat:model` event with the new model ID to update the UI in real-time
- The model change is automatically persisted to the database

:::note UI Dropdown Behavior
The model selector dropdown at the top of the chat is **not** automatically updated when a filter changes the model. This is intentional—the filter may make different routing decisions for each message, so changing the dropdown wouldn't make sense.

If you want to also update the dropdown, you could potentially use JavaScript injection via the event emitter to simulate a click and change the model selection. However, this is generally not recommended for routing filters that may switch models on every message.

**Tip:** You can use the [toggleable filter feature](#-toggleable-filters-vs-always-on-filters) to allow users to enable or disable model routing for a specific filter. This gives users control over when routing is active.

For a complete list of all available event types you can emit, see the [Events documentation](/features/plugin/development/events).
:::
