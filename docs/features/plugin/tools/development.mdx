...
Both External and Built-in Tools support rich UI element embedding, allowing tools to return HTML content and interactive iframes that display directly within chat conversations. This feature enables tools to provide sophisticated visual interfaces, interactive widgets, charts, dashboards, and other rich web content.

When a tool returns an `HTMLResponse` with the `Content-Disposition: inline` header, the content will be embedded as an interactive iframe in the chat interface rather than displayed as plain text.

:::important Native vs External Tools Have Different Return Patterns
The implementation differs based on whether you're writing a native/built-in tool or an external tool (OpenAPI/MCP):

- **Native/Built-in Tools**: Return an `HTMLResponse` object directly
- **External Tools (OpenAPI/MCP)**: Return a tuple `(response, headers)`

Both approaches require the `Content-Disposition: inline` header.
:::

#### Native/Built-in Tools

For native tools, return an `HTMLResponse` object with the `Content-Disposition: inline` header:
...
class Tools:
    ...
    class Valves(BaseModel):
        chart_theme: str = Field("plotly_dark", description="Chart color theme")

    async def create_visualization_tool(self, data: str) -> HTMLResponse:
        """
        Creates an interactive data visualization that embeds in the chat.

        :param data: The data to visualize (comma-separated values)
        """
...
**Key requirements for native tools:**
1. Import `HTMLResponse` from `fastapi.responses`
2. Set return type annotation to `-> HTMLResponse`
3. Use `HTMLResponse(content=..., headers={...})` constructor
4. Include `"Content-Disposition": "inline"` in headers

#### External Tools (OpenAPI/MCP)

For external tools, return a tuple `(response, headers)`:
...
class Tools:
    ...
    class Valves(BaseModel):
        api_endpoint: str = Field("https://api.example.com", description="API endpoint")

    async def create_visualization_tool(self, data: str) -> str:
        """
        Creates an interactive data visualization that embeds in the chat.
...
- Embedded HTML cannot access the parent page's DOM due to cross-origin restrictions
- Communication between iframe and parent requires `postMessage` API
- External resources (scripts, stylesheets) must be loaded via absolute URLs (CDN, etc.)
- Some browser features may be restricted by the iframe sandbox configuration
