...
- **Use Native Mode** when you need reduced latency and system-level tools (Agentic Research, Memory) without complex custom emitter requirements.
...
When **Native Mode** is enabled, Open WebUI automatically injects built-in system tools based on the features enabled for the chat. This enables powerful "Agentic" behaviors where capable models (like GPT-5, Claude 4.5, or MiniMax M2.1) can perform multi-step research or manage user memory dynamically.
...
- **Agentic Research**: Models can invoke `web_search` multiple times to refine results, then use `fetch_url` to read specific deep-dive articles.
- **Dynamic Personalization**: Models can proactively store important facts about the user using `memory_add` during the conversation.
- **Improved Context**: Instead of forcing a search before every prompt, the model decides *when* a search is actually necessary.
