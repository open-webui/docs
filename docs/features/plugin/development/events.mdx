...
| `chat:model`                                 | Update the model displayed for a message (used by routing filters) | `{model: "model-id"}`                                                                    |
...
**Update the model displayed for a message (used by routing filters):**

When a filter routes a message to a different model, emit this event to update the UI so it displays the correct model name for the response.
...
This is typically used in filter `inlet` functions when dynamically routing to different models based on message content or other criteria. See the [Model Routing in Filters](/features/plugin/functions/filter#-model-routing-in-filters) documentation for a complete example.
