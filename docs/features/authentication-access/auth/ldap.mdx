...
LDAP_SERVER_PORT="389"        # Use 389 for plaintext, 636 for LDAPS
LDAP_USE_TLS="false"          # Set to "true" for LDAPS (the default is "true")
LDAP_VALIDATE_CERT="false"    # Set to "true" in production with valid certs (the default is "true")
...
2. Open the **Admin Panel**, go to **Settings** > **Authentication**, and find the **LDAP** section.
3. Turn on the **LDAP** switch.
4. Fill in the fields corresponding to the environment variables above.
5. Save the settings. They apply to the next login attempt, with no restart.
...
Upon successful login, a new user account is created automatically in Open WebUI with the instance's default role, which is `pending` unless an administrator changed it under **Settings** > **Authentication**. The very first account on an instance becomes an administrator. An administrator can later change the user's role if needed.
...
-   **StartTLS is not supported.** `LDAP_USE_TLS` opens an implicit TLS connection only, so `"true"` against a plain listener on port `389` fails. Use LDAPS on `636` or plaintext on `389`.
