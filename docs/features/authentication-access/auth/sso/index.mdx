...
Resolving the issuing client requires RFC 7662 token introspection, which Google, Microsoft Entra ID and GitHub do not implement, so the restriction cannot be applied on those providers.
