...
| `ENABLE_OAUTH_TOKEN_EXCHANGE`         | `false`            | Enables the token exchange endpoint for external apps to exchange OAuth tokens for Open WebUI JWTs. Requires `OAUTH_TOKEN_EXCHANGE_ALLOWED_AUDIENCES`. |
| `OAUTH_TOKEN_EXCHANGE_ALLOWED_AUDIENCES` | *empty*         | Comma-separated OAuth client IDs whose tokens the exchange endpoint accepts. Empty rejects every exchange. |
...
To enable this feature, set `ENABLE_OAUTH_TOKEN_EXCHANGE=true` and list the OAuth clients you trust in `OAUTH_TOKEN_EXCHANGE_ALLOWED_AUDIENCES`. See the [`ENABLE_OAUTH_TOKEN_EXCHANGE`](/reference/env-configuration#enable_oauth_token_exchange) environment variable documentation for details on usage, request/response examples, and security considerations.

:::danger

A successful exchange issues a session as the user the token identifies, with that user's role. Because a provider's userinfo endpoint cannot reveal which OAuth client a token was issued to, the endpoint must be told which clients you trust. Without `OAUTH_TOKEN_EXCHANGE_ALLOWED_AUDIENCES` every exchange is rejected, and listing a client means any holder of a token that client issued can obtain a session as the corresponding user. `OAUTH_ALLOWED_DOMAINS` does not constrain this, because it filters user identities rather than the applications vouching for them.

Resolving the issuing client requires RFC 7662 token introspection, which Google, Microsoft Entra ID and GitHub do not implement.
