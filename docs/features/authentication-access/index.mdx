...
:::tip SSO-Focused UX
In SSO-only environments, you can disable local password auth with `ENABLE_PASSWORD_AUTH=false` and hide account-page password update controls with `ENABLE_PASSWORD_CHANGE_FORM=false`.
:::
