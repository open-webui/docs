...
- Migration `242a2047eae0` temporarily renames the original Text `chat` column to `old_chat`, adds a new JSON `chat` column, copies the data across, then drops `old_chat`; no upgraded database keeps an `old_chat` column.
