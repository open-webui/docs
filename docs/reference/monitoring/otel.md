...
| `OTEL_OTLP_SPAN_EXPORTER`            | `grpc`                          | Exporter protocol: `grpc` (port 4317) or `http` (port 4318); `OTEL_METRICS_OTLP_SPAN_EXPORTER` and `OTEL_LOGS_OTLP_SPAN_EXPORTER` override it per signal |
...
- **FastAPI** (routes)
- **SQLAlchemy** (the sync engine only, which serves startup migrations, config loading and health checks; request-time queries run on the async engine and produce no spans)
- **Redis**
