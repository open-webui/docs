...
This only verifies that the web server is answering: `/health` returns `{"status": true}` unconditionally. `GET /health/db` pings the database, and `GET /ready` returns 503 until startup has completed and the database and Redis answer, so use `/ready` for readiness probes.
