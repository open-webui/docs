...
- Type: `bool`
- Default: `True`
- Description: Enables or disables the Automations feature globally. When disabled, the automation worker loop stops processing scheduled runs, the automation API endpoints return `403 Forbidden`, automation builtin tools are not injected, and the Automations entry is hidden from the sidebar. Requires the `features.automations` user permission (admins always pass).
- Persistence: This environment variable is a `PersistentConfig` variable.
