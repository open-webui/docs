...
- Type: `int`
- Default: unset (no limit)
- Description: Drops a metadata field from what is stored alongside a document chunk when the field is a list or an object whose text runs longer than this many characters. Extraction can attach large structures to a document, and those travel into the vector store with every chunk, where they cost memory and space without helping retrieval. Setting a limit discards the oversized ones and keeps the rest of the metadata intact. Leave unset to store everything as before.
- Persistence: Set via environment variable; applied at startup (not a `ConfigVar`).
...
- **Recommended:** leave it at the default `60`, or raise it toward `120` if you want fewer writes still. Both stay comfortably inside the window.
