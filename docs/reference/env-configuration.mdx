...
This page is up-to-date with Open WebUI release version [v0.11.0](https://github.com/open-webui/open-webui/releases/tag/v0.11.0), but is still a work in progress to later include more accurate descriptions, listing out options available for environment variables, defaults, and improving descriptions.
...
- Description: Sets a default Language Model. Accepts several IDs separated by commas, which pre-selects that set of models on a new chat. Models that no longer exist, or that are hidden, are dropped from a new chat's selection and replaced by these defaults, so a user whose last-used model has been removed still starts on a working one rather than an empty selector. A user's own default model, and a `model` or `models` [URL parameter](/features/chat-conversations/chat-features/url-params), both take precedence and are left alone. In the interface this list is managed per model as **Set as Selected Model** in **Settings > Admin > AI > Models**, see [Selected and Pinned Models](/features/workspace/models#selected-and-pinned-models-admin).
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `str` (JSON array)
- Default: `[]`
- Description: Sets the order models are listed in, as a JSON array of model IDs, for example `["gpt-4o","llama3.1:latest"]`. Models named here appear first in that order, and anything left out follows behind. Previously this could only be arranged in the admin interface, which meant the arrangement was lost on restart wherever configuration is not persisted, so setting it here makes the order survive. Invalid JSON is logged and ignored rather than stopping startup.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `str`
- Default: Empty string (' ')
- Description: Comma-separated list of model IDs to pin by default for users who haven't pinned any models of their own. This gives new accounts a ready-made set of model shortcuts in the sidebar. It seeds the user's own pin list once; after that the user's pins win. In the interface this list is managed per model as **Set as Pinned Model** in **Settings > Admin > AI > Models**, see [Selected and Pinned Models](/features/workspace/models#selected-and-pinned-models-admin).
...
- If parsing fails, Open WebUI logs the error and falls back to `{}`
- Anything that parses to something other than a JSON object (an array or a number, for example) is also replaced by `{}`
...
- Type: `bool`
- Default: `False`
- Description: Swaps the application-wide JSON encoder and decoder from the standard library's `json` module to [orjson](https://pypi.org/project/orjson/), covering HTTP request and response bodies, saving and opening chats, reading settings, the requests sent to model providers, upstream provider responses including streamed chunks, built-in tool results, signing in and signing up, Socket.IO payloads and the stored chunk details read back during knowledge base searches on the Valkey and Oracle 23ai vector stores. Off by default because orjson is stricter about what it accepts; rejected payloads fall back to the standard library automatically. Worth enabling on clustered, Redis-backed deployments and not worth it on single-user instances. Read once at startup, so a restart is required. Added in v0.11.0.
...
- Type: `bool`
- Default: `False`
- Description: Chooses how Open WebUI turns hostnames into addresses for every outbound request. When `False` (the default), it asks the operating system, so it resolves names exactly the way anything else on the machine does. When `True`, it uses the faster c-ares resolver instead, which stops name lookups queueing behind each other and behind other background work. On a busy instance that removes a delay in front of every model call, web search fetch, document fetch and tool call, so it is worth trying. Read once at startup, so a restart is required for a change to take effect. Applies to every outbound request Open WebUI makes.

:::warning Test it before you leave it on

v0.11.0 used c-ares for everything with no way to turn it off, and it does not work everywhere:

- On some Windows hosts it finds no usable nameserver, so every lookup to an external model provider fails.
- In Docker it can intermittently stop resolving container names while Docker's own DNS keeps answering. That empties the Ollama model list and fails in-flight chats with a misleading "Model not found".

The operating system is asked by default again, so a working deployment needs no configuration here. If you turn c-ares on, exercise your models, web search and any internal services afterwards, and turn it back off if lookups start failing.

One more difference to know about: c-ares reads `/etc/resolv.conf` and the hosts file, but not other name sources your system may be configured to use. Names that only those sources know (`.local` via avahi/mDNS, NIS or LDAP directories, Windows NBNS) resolve differently or not at all while it is enabled. `host.docker.internal`, Compose `extra_hosts` and Kubernetes `hostAliases` keep working either way, since those go through the hosts file.

Open WebUI pins the `aiodns` package to version `3.6.1`, the last release before the c-ares build that fails to find a nameserver on Windows. If you build your own image, do not upgrade that package past the pin while this setting is on.
...
`config.contexts` scopes the connection per context. It applies only to orchestrator connections (`server_type` of `orchestrator`, or any connection carrying a `policy_id`) and is ignored everywhere else. Each of the two keys, `chat` and `automation`, accepts:

| Value | Meaning |
| :--- | :--- |
| key omitted, or `{}` | Shared. Every chat (or automation) of the same user reuses one workspace. This is what earlier releases did. |
| `{"context_id": "chat_id"}` | Each saved chat gets its own workspace. Only valid under `chat`. |
| `{"context_id": "automation_id"}` | Each automation gets its own workspace. Only valid under `automation`. |
| `false` | The terminal is not offered in that context, and requests for it are refused. |

Open WebUI sends the resolved context to the orchestrator as an `X-Terminal-Context-Id` header holding `chat:<chat-id>` or `automation:<automation-id>`; shared connections send no header. A chat-scoped terminal needs a saved chat, so it is unavailable in temporary chats and before the first message of a new conversation. See [Terminal Contexts](/features/open-terminal/terminals/orchestration/contexts) for the admin UI and the capacity trade-off.

`auth_type` accepts `bearer` (send `key` as the bearer token), `session` (send the calling user's own Open WebUI token), `system_oauth` (send the user's OAuth access token) and `none`. Use `session` when the orchestrator runs with `TERMINALS_OPEN_WEBUI_URL` set, so it validates each request against the identity of the user who made it. Session auth over the interactive terminal WebSocket applies to orchestrator connections only.
...
- Type: `int`
- Default: `2000`
- Description: Maximum number of characters of `context`-type memories injected into the prompt. The `[Memory Neighborhood]` and `[Relevant Context]` sections share this budget and the text is cut at the limit. Entries are sorted alphabetically, so what falls outside the budget is whatever sorts last, not whatever is oldest.
- Persistence: Set via environment variable; applied at startup (not a `ConfigVar`).
...
- Type: `bool`
- Default: `False`
- Description: Puts a single line describing the shape of the table in front of the parsed contents of every `.csv` file, for example `Table: 501 rows incl. header; 500 data rows; 4 columns: id, name, region, revenue.` The built-in CSV parser emits one document per data row, so without this line nothing in the indexed text says how many rows the file has or which columns exist, and a model asked to count or list them can only go by the rows it retrieved. Column names are taken from the first row, the column count is that of the widest row, and the delimiter is detected from the first 4096 characters of the file, falling back to a comma. The line becomes the first part of the file's extracted content, so it is indexed as an ordinary chunk and is included whenever the whole document is injected. Files that cannot be read, or that have no rows at all, get no summary line and are still parsed as before. Only applies where Open WebUI's own CSV parser is used, so it has no effect when `CONTENT_EXTRACTION_ENGINE` is `external` (everything goes to the external loader) or `tika` or `docling` (a `.csv` is read as plain text); every other engine leaves `.csv` files to the built-in parser. Existing files keep the content they were parsed with, since re-indexing reuses stored text rather than parsing the file again, so re-upload a CSV to give it a summary line. See the [RAG guide](/features/chat-conversations/rag#csv-table-summary).
- Persistence: Set via environment variable; applied at startup (not a `ConfigVar`).
...
- Persistence: This environment variable is a `ConfigVar` variable. It can be configured in the **Settings > Admin > Tools > Web Search > OpenSERP URL** when `openserp` is selected as the search engine.
...
- Type: `bool`
- Default: `True`
- Description: Master switch for signing in through an OAuth or OIDC provider. When set to `False`, both the provider login and the callback are refused, so single sign-on can be turned off without deleting the client ID, secret and endpoint settings. The equivalent toggle sits in **Settings > Admin > System > Authentication**, alongside the LDAP one. Leave it enabled unless you want to suspend SSO while keeping its configuration.
- Persistence: This environment variable is a `ConfigVar` variable.
...
Because the environment is authoritative in that case, the **OAuth / OIDC** section of **Settings > Admin > Authentication** is displayed read-only: every value stays visible but no control accepts input, a note above the section names `ENABLE_OAUTH_PERSISTENT_CONFIG` and saving the page leaves the OAuth settings untouched. Previously the fields were editable and every change was silently dropped on the next restart.

Two consequences while the section is read-only: the client secret stays masked (the reveal button is inert as well); the field values cannot be selected for copying.

To change these settings, edit your environment variables and restart, or set `ENABLE_OAUTH_PERSISTENT_CONFIG=true` to manage them in the Admin Panel.

The read-only state follows this variable alone. Setting it to `True` while [`ENABLE_PERSISTENT_CONFIG`](#enable_persistent_config) is `False` therefore leaves the section editable even though nothing at all is written to the database in that combination, so those edits still apply only to the running process and are gone after a restart.
...
- Type: `bool`
- Default: `False`
- Description: Enables the group management feature. Also configurable in the LDAP section of **Settings > Admin > System > Authentication**.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: If a group from LDAP does not exist in Open WebUI, it will be created automatically. Also configurable in the LDAP section of **Settings > Admin > System > Authentication**.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `str`
- Default: `memberOf`
- Description: Specifies the LDAP attribute that contains the user's group memberships. `memberOf` is a standard attribute for this purpose in Active Directory environments. Also configurable in the LDAP section of **Settings > Admin > System > Authentication**, where group management cannot be saved while this is left empty.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `True`
- Description: Enables or disables user permission to export chats. When disabled, non-admins cannot download a conversation as a file. Shown as **Export Chat** in the group permission list.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `str`
- Default: `True`
- Description: Enables or disables user permission to use the [memory feature](/features/chat-conversations/memory). Set to `False` and the affected users lose the **Personalization** settings tab, the memory API endpoints answer them with `403`, the model is not given the memory tools and their stored memories are no longer injected into the system context of their chats. Admins are exempt from the check.
- Persistence: This environment variable is a `ConfigVar` variable.
...
These settings control whether users can share workspace items **publicly**. All of them default to `False`, and each one has a parent permission that has to be on before its toggle is shown in the admin panel: the matching private-sharing setting above for workspace items and notes, `USER_PERMISSIONS_CHAT_SHARE` for chats and `USER_PERMISSIONS_FEATURES_CALENDAR` for calendars.

:::warning Re-check tools and notes after an upgrade

On instances upgraded from a release that predates `public_tools` and `public_notes`, the admin panel used to draw **Tools Public Sharing** and **Notes Public Sharing** as switched on even though the backend was still refusing the action. Saving any permission on that page then wrote those two as enabled and really did grant public tool and note sharing to everyone. What the admin panel shows now matches what is enforced, but a value already written to the database is not rolled back. Open **Admin Panel → Users → Groups → Default Permissions** and confirm both toggles read the way you intend. See [Permissions](/features/authentication-access/rbac/permissions#2-sharing-permissions).
...
- Type: `bool`
- Default: `False`
- Description: Enables or disables **public sharing** of workspace tools. Requires `USER_PERMISSIONS_WORKSPACE_TOOLS_ALLOW_SHARING` (Tools Sharing) to be enabled for the user: with the parent off, the tool's access-control modal does not offer the "Public" option and the admin panel does not draw this toggle. When disabled, public access grants are silently stripped from tool create and update payloads; per-user and per-group grants are unaffected. Admins always retain the ability to share tools publicly. Configurable per-group in **Admin Panel → Users → Groups → Permissions → Tools Public Sharing**.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: Enables or disables **public sharing** of notes. Requires `USER_PERMISSIONS_NOTES_ALLOW_SHARING` (Notes Sharing) to be enabled for the user: with the parent off, the note's access-control modal does not offer the "Public" option and the admin panel does not draw this toggle. When disabled, public access grants are silently stripped from note create and update payloads; per-user and per-group grants are unaffected. Admins always retain the ability to share notes publicly. Configurable per-group in **Admin Panel → Users → Groups → Permissions → Notes Public Sharing**.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: Enables or disables **public sharing** of chat conversations. "Public" here means every signed-in user of the instance, not the open internet: opening a shared link still requires logging in. For links that work without an account, see [`USER_PERMISSIONS_CHAT_ALLOW_OPEN_SHARING`](#user_permissions_chat_allow_open_sharing). When disabled, the access-control selector in the chat share modal hides the "Public" option for non-admin users. They can still create share links scoped to specific users or groups, but cannot make a chat reachable by everyone. Admins always retain the ability to share chats publicly. Requires `USER_PERMISSIONS_CHAT_SHARE` (Share Chat) to be enabled for the user. Configurable per-group in **Admin Panel → Users → Groups → Permissions → Chats Public Sharing**.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: Allows non-admin users to mark a chat share link **Open**, meaning it can be read by anyone holding the link with no account and no sign-in. This is the only sharing setting that exposes content beyond the instance's own users, so leave it disabled unless publishing conversations to the internet is intended. It grants read access only, and no resource type other than chats accepts this kind of grant. When disabled, the "Open" option is hidden for non-admin users and any open grant sent directly to the API is stripped before saving. Admins are always exempt. Requires `USER_PERMISSIONS_CHAT_SHARE` (Share Chat) to be enabled for the user. Configurable per-group in **Admin Panel → Users → Groups → Permissions → Chats Open Sharing**. See [Open links](/features/chat-conversations/chat-features/chatshare#open-links-no-sign-in).
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Description: Sets the timeout in seconds for the `sio.call()` used in `event_call` events. This controls how long the server waits for a user to respond to interactive prompts, forms, or dropdowns generated by tools and actions using `event_call` (e.g., `execute()` type events). Left unset the server waits indefinitely, so a prompt nobody answers holds that call open. When the timeout elapses, the call returns `{"error": "Event call timed out. The browser tab may be inactive or closed."}` to the calling plugin, so check the returned value for an `error` key rather than wrapping the call in try/except. A value that is not a whole number falls back to `300`.
- Details: The browser session is untouched when the timeout fires, so a tab that is still open and whose user simply has not answered yet stays connected and can be prompted again. Sessions that are genuinely gone are reaped separately, once they stop sending heartbeats. See [Events → Interactive Events](/features/extensibility/plugin/development/events#interactive-events) for how to handle the returned error in a plugin.
