...
- Type: `bool`
- Default: `True`
- Description: Enables the native Anthropic (Claude) provider for direct communication with the Anthropic Messages API.
- Persistence: This environment variable is a `PersistentConfig` variable.
...
- Description: Configures the Anthropic API base URL.
- Persistence: This environment variable is a `PersistentConfig` variable.

#### `ANTHROPIC_API_BASE_URLS`

- Type: `str`
- Description: Supports multiple Anthropic API base URLs, semicolon-separated. Use with `ANTHROPIC_API_KEYS` for multiple connections.
- Example: `https://api.anthropic.com;https://custom-proxy.example.com`
- Persistence: This environment variable is a `PersistentConfig` variable.

#### `ANTHROPIC_API_KEY`

- Type: `str`
- Description: Sets the Anthropic API key.
...
- Type: `str`
- Description: Supports multiple Anthropic API keys, semicolon-separated. Each key corresponds to the base URL at the same index in `ANTHROPIC_API_BASE_URLS`.
- Example: `sk-ant-key-one;sk-ant-key-two`
- Persistence: This environment variable is a `PersistentConfig` variable.
...
1. **Switch to a client-server vector database** such as [PGVector](/reference/env-configuration#pgvector_db_url), Milvus, or Qdrant (recommended).
