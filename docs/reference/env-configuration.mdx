...
The four (_and optionnal fifth_) **user-info** headers are forwarded to nearly every outbound backend. The two **session** headers are narrower:

| Header | Forwarded to |
|---|---|
| `X-OpenWebUI-User-Name`, `X-OpenWebUI-User-Id`, `X-OpenWebUI-User-Email`, `X-OpenWebUI-User-Role`, _`X-OpenWebUI-User-Groups`_ | OpenAI, Ollama and Anthropic model connections, tool servers (OpenAPI and MCP), plus external embedding, reranking, web-search, audio and image services |
...
- Type: `bool`
- Default: `False`
- Description: When `true`, Open WebUI also adds the user groups to the outbounds requests, enabling groups to be used for authorization, rate-limiting or other purposes.

#### `FORWARD_USER_INFO_HEADER_USER_GROUPS`

- Type: `str`
- Default: `X-OpenWebUI-User-Groups`
- Description: Customizes the header name used to forward the user's groups. Only useful if [`ENABLE_FORWARD_USER_INFO_HEADER_USER_GROUPS`](#enable_forward_user_info_header_user_groups) is used.
