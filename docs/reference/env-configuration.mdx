...
- Type: `bool`
- Default: `True`
- Description: Only used in Default Function Calling mode - If True: An LLM generates optimized, distilled search queries from the conversation context. If False: The user's last message is used verbatim as the web search query
- Persistence: This environment variable is a `PersistentConfig` variable.
