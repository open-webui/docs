...
- Type: `int`
- Description: Sets the maximum size of a file in megabytes that can be uploaded for document ingestion. The limit applies to documents Open WebUI fetches itself as well, so a page or file retrieved from a URL is rejected once it exceeds the same size.
- Persistence: This environment variable is a `ConfigVar` variable.
...
Large deployments should raise it further, to somewhere around `300` to `500` seconds, since the only cost is that presence takes longer to go stale.
