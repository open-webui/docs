...
- Type: `bool`
- Default: `False`
- Description: Lets the text of a streamed response grow in place as chunks arrive, instead of copying the whole answer so far on every chunk. The text that reaches the user is identical either way. What it buys you depends entirely on how long your answers get: a normal answer of a few thousand characters saves well under a millisecond across the whole stream, while an answer in the hundreds of thousands of characters is several times faster, because the copying cost grows with the square of the length. What it costs you is a narrow failure mode: the field is emptied while the new chunk is attached and filled back in straight after, so an append that fails (running out of memory part way through a very large response) leaves that message empty instead of holding the text accumulated so far. Nothing else can observe the response mid-update, so the risk is confined to that case. Leave it off unless very long answers show up in your CPU profiles.
- Persistence: Set via environment variable; applied at startup (not a `ConfigVar`).
