...
As of v0.9.6 there is **no hardcoded fallback key**. If you start the backend in a way that skips this step (`open-webui dev`, or invoking `uvicorn` directly, which is unsupported) and authentication is enabled, Open WebUI **refuses to start** and asks you to set `WEBUI_SECRET_KEY` explicitly, instead of silently using an insecure shared default as older versions did.
