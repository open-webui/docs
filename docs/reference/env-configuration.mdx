...
- Default: `{}`
- Description: Custom HTTP response headers to inject into responses returned by the terminal proxy. The terminal proxy strips a fixed set of hop-by-hop and security-sensitive headers from the upstream response; entries in this object are merged in afterward and take precedence on conflicts, so admins can set deployment-specific headers (e.g. sandbox CSP, framing controls, custom referrer policy) on proxied terminal content without touching the application. Invalid JSON falls back to `{}` with no headers added. Example value: `{"Content-Security-Policy": "sandbox allow-scripts allow-same-origin", "Referrer-Policy": "no-referrer"}`.
