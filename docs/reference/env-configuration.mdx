...
Each of these methods checks for the environment variable first, and if it is not set, it generates a random key, saves it to a file and injects it into the environment, all before the application starts. That is why you do not need to set it yourself when you use them.

The Windows script builds its key from the command shell's own `%RANDOM%` generator, which is not a cryptographic source. It is fine for a local install, but set `WEBUI_SECRET_KEY` explicitly for anything you expose to other people. Docker and `open-webui serve` both draw on the operating system's random source instead.
...
- Type: `str`
- Default: The value of the `DATABASE_URL` environment variable
- Description: Sets the connection URL of the openGauss database that stores the embeddings. Leave it unset only if `DATABASE_URL` already points at openGauss.
