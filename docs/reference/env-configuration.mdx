...
- Type: `dict` (JSON object)
- Default: `{}`
- Description: Sets global default parameters (temperature, top_p, max_tokens, seed, etc.) for all models. These defaults are applied as a baseline at chat completion time. Per-model parameter overrides always take precedence. Configurable via **Settings → Admin → Models**.
...
On instances upgraded from a release that predates these two variables, the admin panel used to show **Tools Public Sharing** and **Notes Public Sharing** as switched on even though the action was still being refused. Saving any permission on that page then wrote both as enabled and really did grant public tool and note sharing to everyone. The panel now shows what is enforced, but a value already saved is not rolled back. Open **Settings > Admin > Users > Groups**, edit **Default permissions** and confirm both toggles read the way you intend. See [Permissions](/features/authentication-access/rbac/permissions#2-sharing-permissions).
