...
- Type: `str`
- Options:
- `chroma`, `elasticsearch`, `milvus`, `opensearch`, `pgvector`, `mariadb-vector`, `qdrant`, `pinecone`, `s3vector`, `oracle23ai`, `weaviate`
- Default: `chroma`
...
To use `mariadb-vector`, ensure the MariaDB python driver is installed:

```bash
pip install open-webui[mariadb]
```

Or install all optional vector DB dependencies:
...
MariaDB Vector requires the official MariaDB connector driver and a `mariadb+mariadbconnector://...` URL scheme so vectors can be bound correctly.
...
- Type: `str`
- Default: Empty string (' ')
- Example: `mariadb+mariadbconnector://app:app@127.0.0.1:3306/openwebui`
- Description: Database URL used by the MariaDB Vector backend. **Must** use the `mariadb+mariadbconnector://` scheme.

#### `MARIADB_VECTOR_INITIALIZE_MAX_VECTOR_LENGTH`

- Type: `int`
- Default: `1536`
- Description: Specifies the vector dimension for the MariaDB `VECTOR(n)` column during initialization. Set this to your embedding dimension (e.g., `768` for `nomic-embed-text`).

:::warning

MariaDB Vector cannot change the vector dimension for an existing `VECTOR(n)` column in-place. If you change this value after the table is created you must drop and recreate the backing vector table (and re-ingest/re-embed your documents).
...
- Type: `str`
- Options:
  - `cosine` - Uses `vec_distance_cosine(...)`
  - `euclidean` - Uses `vec_distance_euclidean(...)`
- Default: `cosine`
- Description: Sets the distance function used by MariaDB Vector for similarity search.

#### `MARIADB_VECTOR_INDEX_M`

- Type: `int`
- Default: `8`
- Description: HNSW `M` parameter for the MariaDB `VECTOR INDEX` (number of graph connections). Higher values typically improve recall at the cost of larger indexes and slower builds.

#### `MARIADB_VECTOR_POOL_SIZE`

- Type: `int`
- Default: `None`
- Description: If set, enables a SQLAlchemy connection pool for MariaDB Vector and sets the pool size.

#### `MARIADB_VECTOR_POOL_MAX_OVERFLOW`

- Type: `int`
- Default: `0`
- Description: Maximum number of overflow connections beyond `MARIADB_VECTOR_POOL_SIZE`.
