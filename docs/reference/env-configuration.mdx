...
Open WebUI asks the operating system by default again, so a working deployment needs no configuration here. If you turn c-ares on, exercise your models, web search and any internal services afterwards, and turn it back off if lookups start failing.
...
- Type: `dict` (JSON object)
- Default: `{}`
- Description: The generation parameters sent with background task requests: title generation, tag generation, follow-up suggestions, image prompt generation, retrieval and web search query generation, autocomplete and context compaction summaries. It takes the same keys as a model's advanced parameters (`temperature`, `max_tokens`, `top_p`, `seed`, `reasoning_effort`, `stop`, the Ollama-only options and so on), applied to whichever model ends up running the task. Leave it empty and nothing changes: title generation and compaction summaries stop at 1000 output tokens and the other tasks send no parameters at all. Set anything at all here and that built-in limit is no longer applied, so include `max_tokens` yourself if you still want one. Raising the limit is the fix for a reasoning task model that spends its budget on thinking and returns a title or summary that stops mid-sentence. Emoji generation always uses `max_tokens: 4` and ignores this setting. Can also be set in **Settings > Admin > Interface**, under **Tasks** > **Task Model Parameters** > **Configure**. See [Task Models](/features/administration/task-models) for the full picture.
...
- If parsing fails, Open WebUI logs the error and falls back to `{}`
- These have no effect on background requests, whether you set them here or in the admin panel: `stream_response` (**Stream Chat Response**), `stream_delta_chunk_size` (**Stream Delta Chunk Size**), `function_calling` (**Function Calling**), `reasoning_tags` (**Reasoning Tags**), `compact_token_threshold` (**Context Compaction Threshold**), `system` and `note_id`
...
A chat-scoped terminal needs a saved chat, so it is unavailable in temporary chats and before the first message of a new conversation. See [Terminal Contexts](/features/open-terminal/terminals/orchestration/contexts) for the admin UI and the capacity trade-off.
...
`start.sh` and `open-webui serve` read this as a number of random **bytes** and then base64-encode them, so the default `24` produces a **32-character** key. `backend\start_windows.bat` builds the key one character at a time, so it reads the same value as a number of **characters** and the default `24` produces a **24-character** key. The same setting therefore gives you different keys on different platforms; set it per platform if you need a specific length.

Avoid any value that lands on a key of exactly **44 characters** (`33` on Docker or `open-webui serve`, `44` on Windows). A 44-character `WEBUI_SECRET_KEY` is used verbatim as an encryption key rather than being hashed into one, and an auto-generated key of that length does not decode to the 32 bytes required, so Open WebUI fails to start. The same rule is described under [`ENABLE_VALVE_ENCRYPTION`](#enable_valve_encryption). The default `24` is safe everywhere.

`start.sh` and `open-webui serve` reject a value that is not a positive integer and refuse to start. The Windows script does not validate it.
...
- Type: `bool`
- Default: `False`
- Description: Puts a single line describing the shape of the table in front of the parsed contents of every `.csv` file, naming the total number of rows including the header, the number of data rows and the column names taken from the header row. Without it, nothing in the text a model is given says how many rows or columns the file has, so a question about the size of the table is answered from whichever rows happened to be retrieved. The delimiter is detected from the start of the file, falling back to a comma. The line sits at the top of the file's extracted content, so it is retrieved like any other part of the file and is always present when the whole document is injected. A file that cannot be read, or that has no rows at all, gets no summary line and is parsed as before. No summary is added when `CONTENT_EXTRACTION_ENGINE` is `external`, `tika` or `docling`, which handle `.csv` files themselves; every other engine leaves them to Open WebUI's own CSV parser. Only files parsed after you turn this on get a summary line, and re-indexing does not add one, so re-upload a CSV to give it one. See the [RAG guide](/features/chat-conversations/rag#csv-table-summary).
...
[`ENABLE_PERSISTENT_CONFIG`](#enable_persistent_config) is the one in charge. When it is `False`, nothing is saved to the database whatever `ENABLE_OAUTH_PERSISTENT_CONFIG` says, so setting the OAuth variable to `True` in that case makes the section editable again without making the edits last: they are gone at the next restart. Set both to `True` to manage OAuth from the Admin Panel.
...
- Type: `bool`
- Default: `True`
- Description: Enables or disables user permission to attach web pages (URLs) in chats via the "Attach Webpage" option. When set to `False`, the "Attach Webpage" option stays in the attach menu for non-admin users but is greyed out and does nothing, with a tooltip explaining why, and an attach sent anyway is refused. Also configurable per-group in **Admin Panel > Users > Groups > Permissions > Chat Permissions > Allow Web Upload**.

#### `USER_PERMISSIONS_CHAT_DELETE`

- Type: `bool`
- Default: `True`
- Description: Enables or disables user permission to delete chats. When set to `False`, the delete controls are hidden from non-admins instead of being offered and refused: the chat menu in the sidebar, the `Shift`+hover delete icon on a sidebar row, the chat's own top-bar menu, the search results, the per-chat delete in **Settings > Archived Chats** and **Delete All Chats** in **Settings > Data Controls**. The **Delete chat** keyboard shortcut stops doing anything, since there is no button left for it to press. Archiving is unaffected, and deleting a folder that still holds chats is refused with a permission error because it would take those chats with it. Also configurable per-group in **Admin Panel > Users > Groups > Permissions > Chat Permissions > Allow Chat Delete**.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `True`
- Description: Enables or disables user permission to import chats (uploading a previously exported chat back into Open WebUI). It also covers **forking** a chat, which creates a copy the same way an import does. When set to `False`, non-admins cannot import chats, and the **Fork chat** action under an assistant response and the `/fork` entry in the chat input's `/` menu are hidden rather than shown and refused. Also configurable per-group in **Admin Panel > Users > Groups > Permissions > Chat Permissions > Allow Chat Import**.
- Persistence: This environment variable is a `ConfigVar` variable.
...
On instances upgraded from a release that predates these two variables, the admin panel used to show **Tools Public Sharing** and **Notes Public Sharing** as switched on even though the action was still being refused. Saving any permission on that page then wrote both as enabled and really did grant public tool and note sharing to everyone. The panel now shows what is enforced, but a value already saved is not rolled back. Open **Admin Panel > Users > Groups**, edit **Default permissions** and confirm both toggles read the way you intend. See [Permissions](/features/authentication-access/rbac/permissions#2-sharing-permissions).
