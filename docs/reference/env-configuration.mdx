...
- Type: `bool`
- Default: `True`
- Description: Controls the user status feature, meaning the emoji, message and expiry a user sets on their own account to tell other people what they are up to. When disabled, that control is gone from the user menu and the two endpoints behind it (`GET /api/v1/users/user/status` and `POST /api/v1/users/user/status/update`) answer `403`, so nobody can set or read one.

  This setting does not govern presence. The green activity dot beside a user in channel member lists, on message authors and in the admin user list reports whether that person currently has Open WebUI open, which is a separate signal that stays visible to everyone who can see the user when this setting is off.
...
- Type: `bool`
- Default: `False`
- Description: Controls whether users reach their personal **Integrations** tab in **Settings**, where they add their own tool servers. Off by default, so the tab is out of sight until an admin turns it on under **Settings > Admin > Connections > Direct Integrations**. Turning it on shows the tab to admins and to users holding the `direct_tool_servers` permission; the permission alone is no longer enough. Connections that already exist keep being used while the tab is hidden, so switching this off takes the entry point away without breaking anything already wired up.
- Persistence: This environment variable is a `ConfigVar` variable.
