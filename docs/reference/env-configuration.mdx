...
- Type: `int`
- Default: `50`
- Description: How many matching lines a knowledge search returns before the rest are summarized as a count. It applies to both `grep_knowledge_files` and `kb_exec`'s `grep`, so the two behave alike.
- Persistence: Set via environment variable; applied at startup (not a `ConfigVar`).
...
- Type: `int`
- Default: `100000`
- Description: The hard ceiling, in characters, on what one `view_file` or `view_knowledge_file` call returns. A model asking for more than this is silently held to the ceiling; the response still carries pagination metadata, so it can continue from `next_offset`.
- Persistence: Set via environment variable; applied at startup (not a `ConfigVar`).
...
- Type: `int`
- Default: `10000`
- Description: How much `view_file` and `view_knowledge_file` return when the model does not ask for a size, bounded by [`VIEW_FILE_MAX_CHARS`](#view_file_max_chars). Raising it means fewer round trips through a long document at the cost of a larger first read.
- Persistence: Set via environment variable; applied at startup (not a `ConfigVar`).
...
- Usage: Set to `true` to enable the token exchange endpoint at `/api/v1/oauth/{provider}/token/exchange`. When enabled, external applications can POST to this endpoint with an OAuth access token to obtain an Open WebUI session token. You must also set `OAUTH_TOKEN_EXCHANGE_ALLOWED_AUDIENCES`, or every exchange is rejected.
...
`OAUTH_TOKEN_EXCHANGE_ALLOWED_AUDIENCES` is what restricts this to clients you have chosen to trust. `OAUTH_ALLOWED_DOMAINS` does not: it governs which user identities may sign in, and an impersonated user is a legitimate member of an allowed domain.
...
- The OAuth provider must be properly configured, and must implement RFC 7662 token introspection
- `OAUTH_TOKEN_EXCHANGE_ALLOWED_AUDIENCES` must name the OAuth client the token was issued to
...
- Default: empty
- Description: The OAuth client IDs whose tokens the token exchange endpoint will accept. Open WebUI resolves which client a presented token was issued to using RFC 7662 token introspection, and rejects any token minted for a client that is not listed.
- Usage: `OAUTH_TOKEN_EXCHANGE_ALLOWED_AUDIENCES=internal-cli,reporting-service`

:::warning

An empty value means no client is trusted, so every exchange is rejected. This is deliberate. There is no safe default, because the set of clients permitted to act on your users' behalf is specific to your deployment.
...
**Provider support.** Resolving the issuing client requires RFC 7662 token introspection. Google, Microsoft Entra ID and GitHub do not implement it, so token exchange cannot be used with those providers and rejections are recorded in the server log. Providers that do implement it include Keycloak, Authentik, Okta and Zitadel.
