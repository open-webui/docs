...
http {
    ...
    server {
        ...
        location ~* ^/(auth|api|oauth|admin|signin|signup|signout|login|logout|sso)/ {
            ...
            proxy_buffering off;
            proxy_cache off;
            client_max_body_size 20M; # the app sets no upload cap of its own until Max Upload Size is set in Admin Settings > Documents; this limit is what returns 413
