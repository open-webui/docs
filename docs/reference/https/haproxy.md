...
#OWUI Chat
backend owui_chat
    # add X-Forwarded-For; Open WebUI reads it and X-Forwarded-Proto for client IPs and OAuth redirects
    option forwardfor
...
Socket.IO state lives in each process. To balance across several Open WebUI containers, or to run `UVICORN_WORKERS` above 1, set `WEBSOCKET_MANAGER=redis` with `REDIS_URL` and use a shared database.
:::
