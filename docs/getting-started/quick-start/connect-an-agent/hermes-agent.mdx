...
Open WebUI's connection test requests `GET <URL>/models` with your key, so a URL without the `/v1` suffix normally fails the test itself. If the test passes but no models load, the `/models` response was not JSON with a `data` list (or a bare list), or the connection is switched off. If you filled in Model IDs, only those IDs are listed and `/models` is not queried.
