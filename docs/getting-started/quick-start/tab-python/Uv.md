...
4.  **Remove Data (WARNING: Deletes all data):**
    Delete the directory you set as `DATA_DIR`. Without it, the data was inside the cached tool environment cleared above, which is why the tip above tells you to set one:
    ```bash
    rm -rf ~/.open-webui
    ```
