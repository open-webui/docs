...
Migrations run at import time in every process. With several replicas you can instead set `ENABLE_DB_MIGRATIONS=false` on every replica except one, so only that one runs them; the workers of a single `open-webui serve` process share its environment and cannot be set individually.
:::
