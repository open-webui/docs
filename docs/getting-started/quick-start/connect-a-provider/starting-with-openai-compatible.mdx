...
| Pinstripes | No — endpoint doesn't exist | Add model IDs manually to the whitelist |
...
<Tabs>
  ...
  **OpenRouter** aggregates hundreds of models from multiple providers behind a single API.
  ...
  <TabItem value="pinstripes" label="Pinstripes">

  **[Pinstripes](https://pinstripes.io)** is an OpenAI-compatible inference API offering low-cost access to popular open-weight models with no rate limits and no queues.

  | Setting | Value |
  |---|---|
  | **URL** | `https://pinstripes.io/v1` |
  | **API Key** | Your API key from [pinstripes.io](https://pinstripes.io) |
  | **Model IDs** | **Required** — add manually (see list below) |

  **Available models:**

  | Model ID | Description |
  |---|---|
  | `ps/deepseek-v4-flash` | DeepSeek V4 Flash — $0.10/M input tokens |
  | `ps/glm-4.5-air` | GLM-4.5 Air |
  | `ps/qwen3-35b` | Qwen3 35B |
  | `ps/minimax-m2.7` | MiniMax M2.7 |

  :::caution
  Pinstripes does **not** expose a `/models` endpoint, so auto-detection will fail. You must add model IDs to the **Model IDs (Filter)** allowlist manually before the models appear in your selector.
  :::
