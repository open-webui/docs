...
<Tabs>
  ...
  **LM Studio** provides a local OpenAI-compatible server with a GUI for model management.
  ...
  | **Provider Type** | `LM Studio` (optional, enables model management from Open WebUI) |

  Start the server in LM Studio via the "Local Server" tab before connecting.

  Setting **Provider Type** to `LM Studio` marks loaded models in the model selector and lets admins download, load and unload models without leaving Open WebUI. See [Managing Local Models from Open WebUI](#managing-local-models-from-open-webui).
...
Two provider types go beyond plain chat completions: setting **Provider Type** on the connection tells Open WebUI which management API the server speaks, which unlocks a model management panel for admins.
