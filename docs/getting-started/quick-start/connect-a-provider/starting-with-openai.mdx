...
2. Find the **Manage OpenAI API Connections** list.
3. Click the ➕ **Add Connection** button next to it.
...
<Tabs>
  ...
  *   **API Key**: Your secret key (starts with `sk-...`)
  ...
  1.  Expand **Advanced** and set the **Provider** dropdown to **Azure OpenAI**. An Azure endpoint URL usually flips this on by itself.
...
*   **Model IDs** (under **Advanced**):
    *   *Default (Empty)*: Auto-detects all available models from the provider.
    *   *Set*: Replaces the fetched list. Open WebUI stops calling the provider's `/models` for that connection and lists exactly the IDs you typed, so provider-reported names, sizes and loaded states are gone. Use it to hide older or expensive models, or to name models a provider does not list.
...
*   **Prefix ID**:
    *   If you connect multiple providers that have models with the same name (e.g., two providers both offering `qwen3.6`), add a prefix here (e.g., `groq`) to distinguish them. Open WebUI joins the prefix and the model ID with a dot, so the model appears as `groq.qwen3.6`, and strips the prefix again before the request goes upstream. A prefix typed as `groq/` produces `groq/.qwen3.6`.

*   **Provider**:
    *   Tells Open WebUI what kind of server is behind the URL, which switches on extras such as Azure routing, local model management or Anthropic passthrough. Leave it at **Default** for OpenAI. See [The Provider Setting](/getting-started/quick-start/connect-a-provider/starting-with-openai-compatible#the-provider-setting).
