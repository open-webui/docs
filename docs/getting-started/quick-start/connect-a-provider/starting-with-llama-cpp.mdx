...
2. Go to ⚙️ **Settings → Admin → Connections → OpenAI**.
3. Click ➕ **Add Connection**.
4. Set the following:
   - **URL**: `http://127.0.0.1:10000/v1`
     (Or use `http://host.docker.internal:10000/v1` if running WebUI inside Docker. **Note the `/v1` at the end!**)
   - **Auth**: **None** for a keyless server. With **Bearer** and an empty key, Open WebUI sends an empty bearer token, which some servers reject
   - **Provider**: expand **Advanced** and pick `llama.cpp` from the **Provider** dropdown. This unlocks the loaded-model indicator and the admin **Eject** button in the model selector, plus the admin model management panel described in [Managing Local Models from Open WebUI](/getting-started/quick-start/connect-a-provider/starting-with-openai-compatible#managing-local-models-from-open-webui). See [Unloading Loaded Models](#unloading-loaded-models) below. Leave at `Default` if you do not need those features.
...
For this to work, the matching connection in **Settings → Admin → Connections → OpenAI** must have its **Provider** set to `llama.cpp` (Open WebUI uses that hint to pick the correct unload mechanism). Connections left at the default OpenAI-compatible provider type return an error when an admin attempts to unload a model.

The same **Provider** setting also gives admins a management panel under **Settings > Admin > Models > ⋯ > Manage**, which lists the server's models and can download, load, unload and delete them. See [Managing Local Models from Open WebUI](/getting-started/quick-start/connect-a-provider/starting-with-openai-compatible#managing-local-models-from-open-webui).
