...
- **Pieces of the response arriving out of order.** A fragment of message text, reasoning text or tool call arguments that turns up before the item it belongs to, or that points past the end of the response, is skipped and the rest of the stream carries on. The reply therefore runs to the end rather than breaking off where the stray fragment landed. For a caller hitting `/api/chat/completions` directly, the response body still finishes properly and the [`outlet()` filters](/features/extensibility/plugin/functions/filter) that run once a message is complete still run.
