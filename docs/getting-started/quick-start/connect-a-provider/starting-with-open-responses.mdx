...
2. Under **OpenAI**, click **Manage** (look for the wrench icon).
...
Not every endpoint that advertises the Responses format emits it exactly. Open WebUI absorbs the two most common deviations rather than losing the reply over them, so you should not need to work around either one:
...
- **Pieces of the response arriving out of order.** A fragment of message text, reasoning text or tool call arguments that turns up before the item it belongs to, or that points past the end of the response, is skipped and the rest of the stream carries on. The reply therefore runs to the end rather than breaking off where the stray fragment landed. For a caller hitting `/api/chat/completions` directly, the response body still finishes properly, and the [`outlet()` filters that run when a message completes](/features/extensibility/plugin/functions/filter#filter-behavior-with-api-requests) are not skipped.
