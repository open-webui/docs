...
Native server-side tools run entirely on the provider's side and only work on **API Type: Responses** connections (Steps 1–2 above). They are **independent** of Open WebUI's own [Web Search](/features/chat-conversations/web-search) providers (Tavily, SearXNG, etc.), you don't need to configure or enable those. Leave the model's **Web Search** capability **off**; the native tool replaces it, and enabling both is redundant.
:::
