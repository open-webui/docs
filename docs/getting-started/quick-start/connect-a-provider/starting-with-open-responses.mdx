...
- **A reply that only takes its final shape at the very end.** Some providers skip the events that mark where a message's text begins and ends, and hand over the finished message only once the item is complete. Open WebUI applies that finished message, so the reply is saved in the shape the provider intended: it carries into the conversation history the model sees when you ask your next question, and the source citations that arrive attached to it are kept with it. Providers that emit the full sequence are unaffected, because the finished message matches what was already streamed.
