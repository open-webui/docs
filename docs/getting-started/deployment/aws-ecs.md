...
Set the target group's health-check path to `/ready`, which returns 503 until startup (migrations, config seeding, admin creation) has finished and the database and Redis answer a ping; `/health/db` only pings the database and can return 200 before startup completes. Start with a service health-check grace period of 300 seconds and adjust based on measured startup time. `/health` can be used for a separate process-liveness check. Neither endpoint verifies model APIs or S3 permissions.
...
Open the HTTPS hostname configured in `WEBUI_URL`. The initial administrator credentials create the first account during startup. Once verification passes, increase the desired count and optionally configure Service Auto Scaling. Budget PostgreSQL and Redis connections for the maximum task count; `DATABASE_POOL_SIZE`, `DATABASE_POOL_MAX_OVERFLOW` and `PGVECTOR_POOL_SIZE` set the per-task budget.
