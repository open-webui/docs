...
- Options:
  - `safe_web` - Uses the built-in `SafeWebBaseLoader` for basic, safe web content fetching.
  - `playwright` - Uses Playwright for more advanced web page rendering and interaction. Requires additional configuration.
  - `firecrawl` - Uses the FireCrawl service for crawling and extracting web data. Requires an API key.
  - `tavily` - Uses the Tavily service for in-depth web content extraction. Requires an API key.
  - `external` - Uses a custom, external web loader service.
- Persistence: This environment variable is a `PersistentConfig` variable.

:::info

Each web loader engine has its own set of required environment variables. See the sections below for details on how to configure each one.

:::

#### Playwright Loader (`playwright`)

When `WEB_LOADER_ENGINE` is set to `playwright`, the following variables are used:
...
- **`PLAYWRIGHT_WS_URL`**:
  - Type: `str`
  - Default: `None`
  - Description: The WebSocket URL of a remote Playwright browser instance (e.g., `ws://playwright:3000`). If set, Open WebUI will connect to this remote browser instead of installing dependencies locally.
...
- Connecting to a non-headless (GUI) browser for debugging.

:::

#### FireCrawl Loader (`firecrawl`)

When `WEB_LOADER_ENGINE` is set to `firecrawl`, the following variables are used:

- **`FIRECRAWL_API_KEY`**:
  - Type: `str`
  - Default: `None`
  - Description: Your API key for the FireCrawl service.

- **`FIRECRAWL_API_BASE_URL`**:
  - Type: `str`
  - Default: `https://api.firecrawl.dev`
  - Description: The base URL for the FireCrawl API.

#### Tavily Loader (`tavily`)

When `WEB_LOADER_ENGINE` is set to `tavily`, the following variables are used:

- **`TAVILY_API_KEY`**:
  - Type: `str`
  - Default: `None`
  - Description: Your API key for the Tavily service.

- **`TAVILY_EXTRACT_DEPTH`**:
  - Type: `int`
  - Default: `basic`
  - Description: The depth of content extraction for the Tavily loader.

#### External Loader (`external`)

When `WEB_LOADER_ENGINE` is set to `external`, the following variables are used:

- **`EXTERNAL_WEB_LOADER_URL`**:
  - Type: `str`
  - Default: `None`
  - Description: The URL of your custom external web loader service.

- **`EXTERNAL_WEB_LOADER_API_KEY`**:
  - Type: `str`
  - Default: `None`
  - Description: An optional API key for authenticating with your external web loader service.
