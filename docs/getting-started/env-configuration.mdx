...
- Type: `str`
- Description: The search language when calling [SearXNG search API](https://docs.searxng.org/dev/search_api.html). Examples: `all`, `en`, `en-US`, `de`, `fr-BE` etc.
For complete list, please refere to [sxng_locales.py](https://github.com/searxng/searxng/blob/master/searx/sxng_locales.py)
- Persistence: This environment variable is a `PersistentConfig` variable.
...
- Type: `str`
- Default: `None`
- Description: This sets the [requests timeout](https://requests.readthedocs.io/en/latest/user/advanced/#timeouts) value in **seconds** in case of using the default web loader `safe web`.
Leave empty to use the default requests timeout (very long). Or set a non null positive value in seconds (e.g. `3.05`).
- Persistence: This environment variable is a `PersistentConfig` variable.
...
- Type: `int`
- Default: `2000`
- Description: Sets the timeout in **milliseconds** used by firecrawl [Search](https://docs.firecrawl.dev/features/search). 
Default is 2000ms (meaning 2 seconds). Please refere to [firecrawl Search API](https://docs.firecrawl.dev/api-reference/endpoint/search) for more details.
- Persistence: This environment variable is a `PersistentConfig` variable.
