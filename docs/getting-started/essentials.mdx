...
Open Terminal gives the AI a computer. **[cptr](/features/computer)** ("computer") is the flip side: it gives _you_ your computer in a browser tab. It is a **separate Open WebUI project**, not a feature of this app. cptr is a self-hosted workspace with a terminal, file browser, editor and git that you operate from any device, with an optional AI agent as a helper. Run a project from your phone on the train, then pick it up on your laptop later.

It also connects back here: cptr's OpenAI-compatible gateway exposes each workspace as a model, so you can add a cptr workspace to Open WebUI as a connection and chat with it like any other model.

:::tip When to reach for which
Open Terminal when you want the chat AI to build things for you inside Open WebUI. cptr when you want to _be_ at your own machine remotely, with the AI as an extra pair of hands. They are complementary, not competitors.
:::
