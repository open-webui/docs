...
Open WebUI has two ways of connecting a model to its tools, and **as of v0.9.7 the better one, Native, is the default**. You mostly only need to think about this setting if a particular model lacks a function-calling API.

:::tip Native is the default; Legacy is the opt-out
**Native Mode** is what every chat and model uses unless you change it. **Legacy Mode** (renamed from **Default** in v0.9.7) is the old prompt-based approach, kept only for models that cannot do native function calling. If you were relying on the old behavior, switch those models back to **Legacy**.
:::

When Open WebUI first introduced tool calling, most models did not have built-in function-calling APIs. The only way to give a model tools was to describe them in the system prompt as plain text and then parse the model's response to figure out which tool it was trying to call. This prompt-based approach was originally called **Default**, and it is now renamed **Legacy**: it is the lowest common denominator (any model that can follow a system prompt can use it) but it is slow, breaks KV cache, and cannot reach the built-in system tools.

Most model providers have since added native function-calling support. Open WebUI calls this **Native Mode**: the model receives tool definitions as a structured part of its API request and returns structured tool calls in its response. It is faster, more reliable, preserves KV cache, and is required for all of Open WebUI's built-in system tools (Memory, Notes, Knowledge, Web Search, Image Gen, Code Interpreter). Every major current model supports it (OpenAI, Anthropic, Gemini, Llama 3.1+, Qwen 2.5+, DeepSeek, GLM and others), which is why it is now the default.

You normally do not need to touch this setting. If you do want to pin Native explicitly, or move a model to Legacy, you can set it once for your whole instance:

:::tip Set Function Calling globally in one step
**Admin Panel > Settings > Models** → click the **Settings** button at the **top right** of the models list → set **Function Calling** → save. Every current and future model inherits the setting unless it overrides it.
:::

- **Per model:** **Admin Panel > Settings > Models > [your model] > Model Parameters > Function Calling**
- **Per chat:** In the chat's **Chat Controls** (right sidebar)

If you run an older local model or a fine-tune that does not expose a function-calling API, set that specific model to **Legacy**. For everything else, leave it on Native.
