...
:::note
When using docker via Rancher Desktop, do not pass the flag `--add-host=host.docker.internal:host-gateway` with the `docker run` command as this configuration is Docker Desktop specific and doesn't work with Rancher Desktop.
:::
...
- Download Docker Desktop from [Docker's official website](https://www.docker.com/products/docker-desktop) or an alternate solution such as [Rancher Desktop](https://www.rancherdesktop.io).
- Follow the installation instructions provided on the respective website. After installation, open the application to ensure `docker` is running properly.
