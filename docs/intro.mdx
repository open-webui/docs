...
This command will start a new container named `open-webui` in detached mode, mapping port 3000 on the host machine to port 8080 in the container, and mounting a volume named `open-webui` to persist data.
...
* If you encounter any issues or bugs, please report them to our issue tracker so we can address them promptly.

## Updating

Check out our full [updating guide](/getting-started/updating).

To keep your container up-to-date, you can use Watchtower, a tool that automatically updates running Docker containers. Create a `docker-compose.yml` file with the following contents:
...
You can also update your container manually by running the following commands:
