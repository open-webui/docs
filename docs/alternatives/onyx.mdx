...
- **Permission-aware retrieval from the source system.** Open WebUI's access control decides who may use a knowledge base. It does not inherit per-document permissions from wherever a document came from, so anyone with access to a knowledge base can retrieve from everything in it. Onyx's retrieval respects the source system's own access controls per document, which is a materially different guarantee and the reason the product exists.

If either is a hard requirement, Onyx is the better fit.
