...
On [Onyx Cloud](https://onyx.app/pricing) as of July 2026 the line falls elsewhere again, with role-based access control and permission inheritance included from the Business tier at $20 per user per month, and single sign-on in the Enterprise tier.

On what each project publishes: Onyx lists a price for its Business tier and quotes its Enterprise tier on request. Open WebUI does not publish enterprise pricing at all, and directs you to [contact sales](https://openwebui.com/contact/sales). Neither of us puts an enterprise figure in public, so budget for a conversation either way. The difference that survives that is what you get before any conversation happens, which is the table above.
