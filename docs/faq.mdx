...
**A:** Community support for Open WebUI happens in public channels with help from contributors, volunteers, and members of the core team. These channels are best for shared troubleshooting, reproducible reports, and answers that benefit the wider project. For organizations that need dedicated, guaranteed support, check out our **[Enterprise offerings](https://docs.openwebui.com/enterprise)**.
...
4. **Be kind.** Respectful, well-prepared questions help contributors and core team members respond effectively. Please review our **[Code of Conduct](https://github.com/open-webui/open-webui/blob/main/CODE_OF_CONDUCT.md)** before participating.
...
**A:** If you're using **Ollama**, be aware that Ollama picks a model's default context length from your GPU's VRAM, and on GPUs with less than 24 GiB that is **4096 tokens** (see [Ollama's context length documentation](https://docs.ollama.com/context-length)). This means that none of the retrieved data might be used because it doesn't fit within the available context window.
...
**A:** No. **LLMs do not reliably know their own identity.** When you ask a model "What model are you?" or "Are you GPT-5.6?", the response is not a system diagnostic. It's simply the model generating text based on patterns in its training data.

Models frequently:
- Claim to be a different model (e.g., an open-weight model claiming to be ChatGPT)
...
The model isn't "aware" of itself; it's simply been instructed to claim a specific identity. You can do the same thing in Open WebUI by adding a system prompt to your model configuration (e.g., "You are Qwen 3.6 27B..."). The model will then confidently repeat whatever identity you've told it to claim.
...
Open WebUI is maintained by a small core team. Owning 10+ provider integrations indefinitely would pull core engineering focus away from platform stability, release quality, and the features used across all providers.
