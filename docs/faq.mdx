...
#### **Q: What is the difference between Direct and External connections in Open WebUI?**

**A:** Open WebUI offers two ways to connect to LLM APIs:

- **External Connections**: The central backend server sends API requests to LLM providers on behalf of users. External connections generally cannot access services running locally on your machine unless specifically configured to allow it (e.g., if both the backend and local LLM are on the same machine and properly routed). These connections can be added from the **`Admin Panel > Settings > Connections`**.

- **Direct Connections**: The browser sends API requests directly to the LLM provider, bypassing the backend server. This allows interaction with local or external services from the users' computer. These connections can be added from the **`Settings > Connections`**.
