...
```
#### **Q: Why I can't see any model shown in the model list even I've installed models in ollama? (models list is empty)?**

**A:** You need to modify the system service to add the proper environmental variable to serve ollama over the lan network.
1. Edit the service file: 
```bash
sudo nano /etc/systemd/system/ollama.service 
```
2. Then add the following line inside the **[Service]** section:
```bash
Environment="OLLAMA_HOST=0.0.0.0"
```
3. reload the daemons using the command, then restart the service 
```bash
