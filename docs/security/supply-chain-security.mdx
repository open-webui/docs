...
This is intentional, layered design: server-side code execution is gated behind explicit administrator configuration and an off-by-default permission, rather than available to every user. See the [Tools and Functions security model](/features/extensibility/plugin/tools) and the [Security Policy](./security-policy) for the full model.
